...
const STATUS_MAP: Record<string, { label: string; color: string; icon: any }> = {
  new: { label: "لم تبدأ", color: "bg-muted text-muted-foreground border-border", icon: ListTodo },
  progress: { label: "قيد التنفيذ", color: "bg-blue-500/10 text-blue-400 border-blue-500/30", icon: Clock },
  hold: { label: "معلقة", color: "bg-amber-500/10 text-amber-400 border-amber-500/30", icon: Pause },
  done: { label: "مكتملة", color: "bg-green-500/10 text-green-400 border-green-500/30", icon: CheckCircle2 },
  cancelled: { label: "ملغاة", color: "bg-red-500/10 text-red-400 border-red-500/30", icon: XCircle },
};

const PRIORITY_MAP: Record<string, { label: string; color: string }> = {
  high: { label: "عالية", color: "bg-red-500/10 text-red-400" },
  medium: { label: "متوسطة", color: "bg-amber-500/10 text-amber-400" },
  low: { label: "منخفضة", color: "bg-green-500/10 text-green-400" },
};
    ...
    <div className="min-h-screen bg-background" dir="rtl">
        ...
        <div className="max-w-7xl mx-auto px-6 py-5">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-foreground">لوحة تحكم الوكلاء</h1>
                <p className="text-sm text-muted-foreground">متابعة وإدارة فريق الوكلاء الذكيين</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <Button
                ...
                className="border-border/50 text-muted-foreground hover:text-primary hover:border-primary/30 bg-transparent"
              >
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
          {[
            { label: "إجمالي المهام", value: statsData.total, color: "text-primary" },
            { label: "لم تبدأ", value: statsData.new, color: "text-muted-foreground" },
            { label: "قيد التنفيذ", value: statsData.progress, color: "text-blue-400" },
            { label: "معلقة", value: statsData.hold, color: "text-amber-400" },
            { label: "مكتملة", value: statsData.done, color: "text-green-400" },
            { label: "ملغاة", value: statsData.cancelled, color: "text-red-400" },
            { label: "متأخرة", value: statsData.overdue, color: "text-orange-400" },
            { label: "نسبة الإنجاز", value: `${statsData.completionRate}%`, color: "text-emerald-400" },
          ].map((stat, i) => (
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-card border border-border/50">
            <TabsTrigger value="agents" className="gap-1.5 data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
              <Bot className="w-4 h-4" />
              الوكلاء ({agentsList.length})
            </TabsTrigger>
            <TabsTrigger value="tasks" className="gap-1.5 data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
              <ListTodo className="w-4 h-4" />
              المهام ({filteredTasks.length})
            </TabsTrigger>
            <TabsTrigger value="activity" className="gap-1.5 data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
              <Activity className="w-4 h-4" />
              سجل النشاط ({(agentActivity as any[]).length})
            </TabsTrigger>
            <TabsTrigger value="projects" className="gap-1.5 data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary border border-primary/20">المنسقة</span>
                ...
                return (
                  <div key={agent.id} className="glass-card p-5 group">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className={`w-2 h-2 rounded-full shrink-0 ${agent.status === "active" ? "bg-green-500" : agent.status === "maintenance" ? "bg-amber-500" : "bg-gray-500"}`} />
                    </div>

                    <p className="text-xs text-muted-foreground/70 leading-relaxed mb-3 line-clamp-2">{agent.description}</p>

                    {agentTaskCount > 0 ? (
                      <div className="space-y-2">
                        <div className="grid grid-cols-3 gap-2 text-center text-xs">
                          <div className="bg-card rounded-lg p-1.5 border border-border/30">
                            <div className="font-bold text-foreground">{agentTaskCount}</div>
                            <div className="text-muted-foreground/60 text-[10px]">مهام</div>
                          </div>
                          <div className="bg-card rounded-lg p-1.5 border border-border/30">
                            <div className="font-bold text-green-400">{agentDoneCount}</div>
                            <div className="text-muted-foreground/60 text-[10px]">مكتملة</div>
                          </div>
                          <div className="bg-card rounded-lg p-1.5 border border-border/30">
                            <div className="font-bold text-primary">{agentCompletionRate}%</div>
                            <div className="text-muted-foreground/60 text-[10px]">إنجاز</div>
                          </div>
                        </div>
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      className="w-full mt-3 text-xs border-border/30 text-muted-foreground hover:text-primary hover:border-primary/30 bg-transparent"
                      onClick={() => {
            ...
            <div className="glass-card p-5 mt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={i} className="bg-card rounded-lg p-3 border border-border/30 flex items-center gap-2">
                    <Badge className={`text-[10px] px-1.5 ${api.method === "POST" ? "bg-green-500/20 text-green-400 border-green-500/30" : "bg-blue-500/20 text-blue-400 border-blue-500/30"}`}>
                      {api.method}
                    </Badge>
                    <code className="text-[11px] text-muted-foreground font-mono">{api.path}</code>
                    <span className="text-[10px] text-muted-foreground/60 mr-auto">{api.desc}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="glass-card p-4 mb-4">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Select value={filterAgent} onValueChange={setFilterAgent}>
                  <SelectTrigger className="w-[160px] h-9 text-sm bg-card border-border/50">
                ...
                <Select value={filterPriority} onValueChange={setFilterPriority}>
                  <SelectTrigger className="w-[140px] h-9 text-sm bg-card border-border/50">
                ...
                <div className="relative flex-1 min-w-[200px]">
                  ...
                  <Input
                    ...
                    className="pr-9 h-9 text-sm bg-card border-border/50"
                  />
                </div>
            ...
            <div className="glass-card overflow-hidden">
              {isLoading ? (
                ...
                <div className="text-center py-16">
                  <Bot className="w-12 h-12 mx-auto mb-3 text-muted-foreground/20" />
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {filteredTasks.map((task: any) => {
                        ...
                        return (
                          <tr key={task.id} className={`border-b border-border/20 hover:bg-card/50 transition-colors ${isOverdue ? "bg-red-500/5" : ""}`}>
                            <td className="p-3">
                            ...
                            <td className="p-3">
                              <Badge variant="outline" className="text-xs font-normal border-border/50 text-muted-foreground">{task.project}</Badge>
                            </td>
                            <td className="p-3">
                    ...
                    {(agentActivity as any[]).map((a: any, i: number) => {
                      ...
                      return (
                        <div key={i} className="relative flex gap-4">
                          ...
                          <div className="flex-1 bg-card rounded-lg border border-border/30 p-3">
                            ...
                            {a.project && (
                              <Badge variant="outline" className="text-[10px] mt-1.5 border-border/50 text-muted-foreground">{a.project}</Badge>
                            )}
                          </div>
                        </div>
                      );
              ...
              {projectDistribution.length === 0 ? (
                ...
                <div className="space-y-3">
                  {projectDistribution.map(([project, count], i) => {
                    ...
                    return (
                      <div key={project} className="bg-card rounded-lg p-4 border border-border/30">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center gap-3 text-sm">
                            ...
                            <Badge variant="outline" className="text-[10px] border-border/50 text-muted-foreground">{percentage}%</Badge>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
