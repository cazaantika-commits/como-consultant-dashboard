...
export default function OfferReaderScreen({ cpaProjectId, projectConsultantId, systemProjectId, consultantName, onBack }: { cpaProjectId: number; projectConsultantId: number; systemProjectId: number; consultantName: string; onBack: () => void }) {
  ...
  const saveOwnerCorrectionMutation = trpc.offerReader.saveOwnerCorrection.useMutation({ onSuccess: () => { readingsQuery.refetch(); utils.financialOfferComparison.invalidate(); } });
  const [proposalId, setProposalId] = useState("");
  const [isEditing, setIsEditing] = useState(false);
  const [isOwnerCorrection, setIsOwnerCorrection] = useState(false);
  ...
  const beginManualReview = () => {
    ...
    setIsOwnerCorrection(Boolean(extracted));
    setIsEditing(true);
  };
  ...
  const saveManualReview = () => {
    ...
    const mutation = isOwnerCorrection ? saveOwnerCorrectionMutation : saveReviewMutation;
    mutation.mutate({ readingId: latest.id, extraction }, { onSuccess: () => { setIsEditing(false); setIsOwnerCorrection(false); } });
  };
