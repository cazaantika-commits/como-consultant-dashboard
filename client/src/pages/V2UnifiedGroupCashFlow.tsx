      ...
      <div className="mx-auto max-w-[1640px]">
        ...
        <section className="mt-4 overflow-hidden rounded-2xl border-2 border-slate-300 bg-white shadow-sm">
          ...
          <div className="overflow-x-auto">
            <table className="w-max min-w-[820px] border-separate border-spacing-0 text-[11px]" style={{ minWidth: Math.max(820, report.rows.length * 160 + 310) }}>
              <thead><tr className="bg-slate-900 text-white"><th className="sticky right-0 z-20 min-w-[126px] border-l border-slate-600 bg-slate-900 px-3 py-2.5 text-right font-black">الشهر</th>{report.rows.map((row) => <th key={row.projectId} className={`min-w-[150px] border-l border-slate-600 px-3 py-2.5 text-center font-black ${row.sourceKind === "commercial_development" ? "bg-cyan-900" : ""}`}><span className="block leading-4">{row.name}</span>{row.sourceKind === "commercial_development" && <span className="mt-1 inline-block rounded bg-cyan-50/15 px-1.5 py-0.5 text-[9px] font-bold text-cyan-100">تطوير قبل التشغيل</span>}</th>)}<th className="min-w-[128px] border-r-2 border-slate-400 bg-teal-800 px-3 py-2.5 text-center font-black">إجمالي المجموعة</th><th className="min-w-[135px] border-r border-slate-600 bg-slate-800 px-3 py-2.5 text-center font-black">التراكمي</th></tr></thead>
              <tbody>
                <tr className="border-b-2 border-slate-700 bg-slate-100"><td className="sticky right-0 z-10 border-l border-slate-300 bg-slate-100 px-3 py-2 text-right font-black text-slate-900">مدفوع مسبقًا</td>{report.rows.map((row) => { const paid = report.projects.find((project) => project.projectId === row.projectId)?.paidBeforeSchedule || 0; return <td key={row.projectId} className="border-l border-slate-200 px-3 py-2 text-center font-black tabular-nums text-red-800">{paid > 0 ? `−${unsignedAmount(paid)}` : "—"}</td>; })}<td className="border-r-2 border-slate-400 bg-slate-200 px-3 py-2 text-center font-black tabular-nums text-red-900">{report.paidBeforeScheduleTotal > 0 ? `−${unsignedAmount(report.paidBeforeScheduleTotal)}` : "—"}</td><td className="border-r border-slate-300 bg-slate-200 px-3 py-2 text-center font-black tabular-nums text-red-900">{report.paidBeforeScheduleTotal > 0 ? `−${unsignedAmount(report.paidBeforeScheduleTotal)}` : "—"}</td></tr>
