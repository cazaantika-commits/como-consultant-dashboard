...
- [ ] Resolve the Commercial Center design-scope integrity mismatch without overwriting owner choices: current data contains 35 selected items and APPROVED status while its historical note and prior snapshot record state 39 items and DRAFT; preserve the 35 selections until the owner confirms the intended scope and synchronize the status and audit note afterward.
