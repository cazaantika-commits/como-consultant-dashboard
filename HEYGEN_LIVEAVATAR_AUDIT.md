...
## Reusing Salwa's current approved image — official requirements

HeyGen's official Custom LiveAvatar guide confirms that a single JPEG or PNG can become a fully interactive real-time avatar from the Avatars section. The source must be at least 1080p and use a 16:9 or 9:16 aspect ratio; the preferred composition is one centered, chest-up person, looking straight at the camera, with the mouth closed and no hands or face obstructions. The current approved Salwa image is 2048×2048, so its resolution passes but its square aspect ratio does not; it needs a content-preserving 9:16 adaptation before upload. The same official help center says LiveAvatar is a separate platform and subscription from HeyGen, custom avatar access depends on the LiveAvatar plan, and custom processing is estimated at about 24 hours.

Sources: https://help.heygen.com/en/articles/9612935-liveavatar-custom-liveavatar-creation-guide and https://help.heygen.com/en/articles/10035615-how-to-get-started-with-liveavatar

## Marianne candidate test — 2026-09-08

The public preset `Marianne in Red Suit` (`8532b602-89e8-44fa-a9e2-5a4259a058cc`) was created successfully as a Sandbox embed with Salwa's Arabic context. It rendered correctly and Arabic was the selected language, but after the user-initiated `Chat now` step the session remained at `Connecting...` and ended with “Something went wrong. Please try again later.” The candidate is not approved and was not installed in COMO. The next diagnostic will use the preset’s own official voice agent in Sandbox, without Production use.

The follow-up Sandbox used Marianne’s own official Voice Agent (`8635cd96-e8e8-40b0-a048-e14dfae81561`) without Salwa’s context. The page switched the selected language to English and still ended with the same session error after `Connecting...`. This rules out Salwa’s Arabic context as the cause for this candidate. The account’s visible preset library can render embedding pages, but no further preset will be tried until the session-start entitlement or provider error is resolved.
