...
import { ArrowRight, Layers, ChevronDown, ChevronUp, RotateCcw, Settings, Calendar, Clock, Save, X, Eye, EyeOff, LayoutGrid, Rows3, TrendingUp, DollarSign, AlertTriangle, Target, BarChart3, RefreshCw } from "lucide-react";
...
// ── Phase colors (dark muted on white canvas) ──────────────────────────────
// Dark ink on white paper: calm, professional, executive-grade
const PHASE_COLORS = {
...
export default function CapitalSchedulingPage({ onBack }: Props) {
  ...
  const [availableCapital, setAvailableCapital] = useState<number>(100_000_000);
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          ...
          <button
            ...
            style={{
              display: "flex", alignItems: "center", gap: 6,
              background: showSettings ? "#c2410c" : "#ffffff",
              color: showSettings ? "#fff" : "#64748b",
              border: "1px solid " + (showSettings ? "#c2410c" : "#e2e8f0"),
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <Settings style={{ width: 16, height: 16, color: "#c2410c" }} />
                        ...
                        <>
                          ...
                          <td style={{ padding: "8px 12px", textAlign: "center" }}>
                            <button
                              ...
                              style={{
                                background: "#fff7ed", color: "#c2410c", border: "1px solid #fdba74", borderRadius: 8,
                                padding: "4px 12px", cursor: "pointer", fontSize: 11, fontWeight: 700,
                                display: "inline-flex", alignItems: "center", gap: 4,
                              }}
                            >
      ...
      {(() => {
        // Compute summary metrics from effectiveColumns
        const totalCosts = effectiveColumns.reduce((s, c) => s + c.grandTotal, 0);
        const totalPaid = effectiveColumns.reduce((s, c) => s + c.paidTotal, 0);
        const totalUpcoming = effectiveColumns.reduce((s, c) => s + c.upcomingTotal, 0);
        // Max exposure = peak cumulative outflow
        const maxCumulative = cumulativeTotals.length > 0 ? Math.max(...cumulativeTotals) : 0;
        const maxExposure = totalPaid + maxCumulative;
        // Find the month of max exposure
        const maxCumIdx = cumulativeTotals.indexOf(Math.max(...cumulativeTotals));
        const maxExposureMonth = maxCumIdx >= 0 ? getMonthLabel(maxCumIdx * groupBy) : "";
        // Net profit placeholder (revenues - costs, for now show as % of costs)
        const totalRevenues = totalCosts * 1.04; // Placeholder: assume 4% margin
        const netProfit = totalRevenues - totalCosts;
        const roi = totalCosts > 0 ? ((netProfit / totalCosts) * 100).toFixed(1) : "0";
        // Funding gap
        const fundingGap = maxExposure > availableCapital ? maxExposure - availableCapital : 0;
        // Deficit months
        let deficitMonths = 0;
        let runningCum = 0;
        for (const row of groupedRows) {
          runningCum += row.total;
          if (totalPaid + runningCum > availableCapital) deficitMonths++;
        }

        const summaryCards = [
          {
            title: "إجمالي التكاليف",
            value: fmtBig(totalCosts),
            subtitle: "AED",
            borderColor: "#ef4444",
            bgGradient: "linear-gradient(135deg, #fef2f2 0%, #fff 100%)",
            iconBg: "#ef4444",
            icon: <TrendingUp style={{ width: 20, height: 20, color: "#fff" }} />,
          },
          {
            title: "إجمالي المطلوب",
            value: fmtBig(totalUpcoming),
            subtitle: "AED",
            borderColor: "#10b981",
            bgGradient: "linear-gradient(135deg, #ecfdf5 0%, #fff 100%)",
            iconBg: "#10b981",
            icon: <BarChart3 style={{ width: 20, height: 20, color: "#fff" }} />,
          },
          {
            title: "أقصى تعرض للمستثمر",
            value: fmtBig(maxExposure),
            subtitle: maxExposureMonth,
            borderColor: "#f59e0b",
            bgGradient: "linear-gradient(135deg, #fffbeb 0%, #fff 100%)",
            iconBg: "#f59e0b",
            icon: <Target style={{ width: 20, height: 20, color: "#fff" }} />,
          },
          {
            title: "المدفوع حتى الآن",
            value: fmtBig(totalPaid),
            subtitle: "AED",
            borderColor: "#6366f1",
            bgGradient: "linear-gradient(135deg, #eef2ff 0%, #fff 100%)",
            iconBg: "#6366f1",
            icon: <DollarSign style={{ width: 20, height: 20, color: "#fff" }} />,
          },
          {
            title: "رأس المال المتاح",
            value: fmtBig(availableCapital),
            subtitle: "AED",
            borderColor: "#3b82f6",
          ...
          {
            title: "فجوة التمويل",
            value: fundingGap > 0 ? fmtBig(fundingGap) : "لا يوجد",
            subtitle: fundingGap > 0 ? `${deficitMonths} شهر عجز` : "وضع مريح",
            borderColor: fundingGap > 0 ? "#ef4444" : "#10b981",
            bgGradient: fundingGap > 0 ? "linear-gradient(135deg, #fef2f2 0%, #fff 100%)" : "linear-gradient(135deg, #ecfdf5 0%, #fff 100%)",
            iconBg: fundingGap > 0 ? "#ef4444" : "#10b981",
            icon: <AlertTriangle style={{ width: 20, height: 20, color: "#fff" }} />,
          },
        ];
        ...
        return (
          <div style={{ marginBottom: 24 }}>
            ...
            {/* Project Control + Available Capital Row */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 16 }}>
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                  <Layers style={{ width: 18, height: 18, color: "#64748b" }} />
                  <span style={{ fontSize: 15, fontWeight: 800, color: "#0f172a" }}>التحكم بالمشاريع</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {effectiveColumns.map((col) => {
                    const delay = getDelay(col.projectId);
                    const hasDelay = delay.designDelay > 0 || delay.offplanDelay > 0 || delay.constructionDelay > 0;
                    const totalMonths = col.preDevMonths + col.constructionMonths + col.handoverMonths;
                    // Assign a color dot per project
                    const dotColors = ["#ef4444", "#3b82f6", "#f59e0b", "#10b981", "#8b5cf6", "#ec4899"];
                    const dotColor = dotColors[effectiveColumns.indexOf(col) % dotColors.length];
                    return (
                      <div
                        key={col.projectId}
                        style={{
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: 11, color: "#94a3b8", display: "flex", gap: 8, flexWrap: "wrap" }}>
                              <span>{col.startDate}</span>
                              <span>·</span>
                              <span>{totalMonths} شهر</span>
                              <span>·</span>
                              <span style={{ color: "#c2410c", fontWeight: 600 }}>{fmtBig(col.grandTotal)}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          {hasDelay ? (
                            <div style={{
                              ...
                              {delay.designDelay > 0 && `تصاميم +${delay.designDelay}`}
                              {delay.offplanDelay > 0 && ` أوف +${delay.offplanDelay}`}
                              {delay.constructionDelay > 0 && ` إنشاء +${delay.constructionDelay}`}
                            </div>
                          ) : (
                            <div style={{
                          ...
                          {/* Plus/Minus for design delay quick control */}
                          <button
                            onClick={() => adjustDelay(col.projectId, "designDelay", 1)}
                            style={{
                          ...
                          <button
                            onClick={() => adjustDelay(col.projectId, "designDelay", -1)}
                            disabled={delay.designDelay === 0}
                            style={{
                              width: 28, height: 28, borderRadius: 8,
                              background: delay.designDelay === 0 ? "#f9fafb" : "#f1f5f9",
                              border: `1px solid ${delay.designDelay === 0 ? "#f3f4f6" : "#e2e8f0"}`,
                              cursor: delay.designDelay === 0 ? "default" : "pointer",
                              display: "flex", alignItems: "center", justifyContent: "center",
                              fontSize: 16, color: delay.designDelay === 0 ? "#d1d5db" : "#64748b", fontWeight: 700,
                            }}
                            title="تقديم شهر"
                          >
                            −
                          ...
                          <button
                            ...
                            title={hiddenProjects.has(col.projectId) ? "إظهار المشروع" : "إخفاء المشروع"}
                          >
                            {hiddenProjects.has(col.projectId) 
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                  ...
                  <span style={{ fontSize: 15, fontWeight: 800, color: "#0f172a" }}>رأس المال المتاح</span>
                </div>

                <input
                  type="text"
                  value={fmtComma(availableCapital)}
                  onChange={(e) => {
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center" }}>
                  {[
                    ...
                    <button
                      key={preset.value}
                      onClick={() => setAvailableCapital(preset.value)}
                      style={{
                        padding: "8px 18px",
                        borderRadius: 10,
                        fontSize: 13,
                        ...
                        cursor: "pointer",
                        transition: "all 0.2s",
                        border: availableCapital === preset.value ? "2px solid #3b82f6" : "1px solid #e5e7eb",
                        background: availableCapital === preset.value ? "#eff6ff" : "#ffffff",
                        color: availableCapital === preset.value ? "#2563eb" : "#64748b",
                      }}
                    >
                      {preset.label}
                    </button>
                ...
                <div style={{ marginTop: 20, padding: "12px 14px", background: "#f8fafc", borderRadius: 10, border: "1px solid #f1f5f9" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                    <span style={{ fontSize: 11, color: "#94a3b8" }}>أقصى تعرض</span>
                    <span style={{ fontSize: 12, fontWeight: 700, color: "#f59e0b" }}>{fmtBig(maxExposure)}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                    <span style={{ fontSize: 11, color: "#94a3b8" }}>رأس المال المتاح</span>
                    <span style={{ fontSize: 12, fontWeight: 700, color: "#3b82f6" }}>{fmtBig(availableCapital)}</span>
                  </div>
                  <div style={{
                    height: 1, background: "#e5e7eb", margin: "8px 0",
                  }} />
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 11, color: "#94a3b8" }}>الفجوة</span>
                    <span style={{
                      fontSize: 13, fontWeight: 800,
                      color: fundingGap > 0 ? "#ef4444" : "#10b981",
                    }}>
                      {fundingGap > 0 ? fmtBig(fundingGap) : "لا يوجد عجز"}
                    </span>
                  </div>
                </div>
              </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{
                        ...
                        height: 22,
                        background: "#c2410c", borderRadius: 6,
                        padding: "0 6px", marginTop: 4,
                        fontSize: 9,
                      }}>
                        <span style={{ color: "#fed7aa", fontWeight: 600 }}>الإجمالي</span>
                      ...
                      <div style={{
                        display: "flex", justifyContent: "space-between", alignItems: "center",
                        height: 22,
                        background: "#166534", borderRadius: 6,
                        padding: "0 6px", marginTop: 3, marginBottom: 6,
                        fontSize: 9,
                      }}>
                        <span style={{ color: "#bbf7d0", fontWeight: 600 }}>المطلوب</span>
                ...
                <tr key={row.gi}>
                  ...
                  <td
                    style={{
                      ...
                      minWidth: COL_W,
                      height: ROW_H,
                      background: row.total > 0 ? "#c2410c" : (isEven ? "#ffffff" : "#fafafa"),
                      padding: 0,
                      textAlign: "center",
                      fontSize: 11,
                  ...
                  <td
                    style={{
                      ...
                      minWidth: COL_W,
                      height: ROW_H,
                      background: cumulativeTotals[row.gi] > 0 ? "#1e3a5f" : (isEven ? "#ffffff" : "#fafafa"),
                      padding: 0,
                      paddingRight: GAP,
                      textAlign: "center",
                      fontSize: 11,
