          ...
          <>
            <section className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_370px]">
              <div className="min-w-0 space-y-4">
                ...
                <section data-testid="collection-cash-reading" className="overflow-hidden rounded-[22px] border border-slate-300 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.07)]">
                  <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 bg-slate-50/70 px-5 py-4"><div><p className="text-[11px] font-bold text-blue-700">قراءة مرئية دائمًا</p><h2 className="mt-0.5 text-lg font-black text-slate-900">{isBuildForSale ? "الاستلام المباشر والأثر النقدي" : "التحصيل والإسكرو حسب شهر البيع"}</h2><p className="mt-1 text-xs leading-5 text-slate-600">{isBuildForSale ? "التحصيل الفعلي يساوي قيمة البيع لأن كامل حصيلة الوحدة تدخل للمستثمر عند البيع بعد الإنجاز." : "التحصيل الفعلي هو ما دخل إلى الإسكرو من الدفعات، وقد يختلف عن قيمة الوحدات المباعة في الشهر نفسه."}</p></div><Badge className={isBuildForSale ? "border border-indigo-200 bg-indigo-50 text-indigo-800 hover:bg-indigo-50" : hasDeficit ? "border border-red-200 bg-red-50 text-red-800 hover:bg-red-50" : "border border-emerald-200 bg-emerald-50 text-emerald-800 hover:bg-emerald-50"}>{isBuildForSale ? "استلام مباشر" : hasDeficit ? "تنبيه سيولة إسكرو" : "سيولة الإسكرو مريحة"}</Badge></div><div className="grid gap-2.5 p-4 sm:grid-cols-2 xl:grid-cols-4">{cashInflowData.filter((row) => row.cashInflow > 0 || row.salesThisMonth > 0).slice(0, 8).map((row) => <article key={row.month} className="rounded-2xl border border-slate-300 bg-white p-3.5"><div className="flex items-center justify-between"><p className="text-xs font-black text-slate-800">شهر {row.month}</p><span className="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-bold text-blue-800">{isBuildForSale ? "مباشر" : "إسكرو"}</span></div><p className="mt-3 text-[10px] font-bold text-slate-600">{isBuildForSale ? "الاستلام الفعلي" : "التحصيل الفعلي"}</p><p className="mt-1 text-lg font-black tabular-nums text-blue-700">{fmt(row.cashInflow)} <span className="text-[10px]">AED</span></p><div className="mt-3 border-t border-slate-200 pt-2"><p className="text-[10px] font-bold text-slate-600">قيمة البيع في الشهر</p><p className="mt-1 text-sm font-black tabular-nums text-slate-800">{fmt(row.salesThisMonth)} <span className="text-[10px]">AED</span></p></div></article>)}</div>
                </section>
