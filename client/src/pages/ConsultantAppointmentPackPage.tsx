      ...
      <main className="mx-auto w-full min-w-0 max-w-6xl px-4 py-6 sm:px-6 sm:py-9">
        ...
        {pack && <section className="mt-6 space-y-5">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <article className="rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><FileCheck2 className="h-4 w-4 text-emerald-700" /><h4 className="font-extrabold text-foreground">4. نطاق التكليف المطلوب</h4></div><SourceTag>نطاق المشروع المستقل</SourceTag></div>{pack.sections.scope.itemCount > 0 ? <><p className="mt-3 text-sm font-bold text-foreground">{pack.sections.scope.itemCount} بندًا مختارًا لهذا المشروع</p><p className="mt-1 text-xs leading-5 text-muted-foreground">هذه معاينة لاختيارات المشروع من المكتبة الشاملة، ولا تعدل البنود أو نطاق أي مشروع آخر.</p><div className="mt-3 space-y-2">{pack.sections.scope.sections.slice(0, 3).map((section: any) => <div key={section.label} className="rounded-xl bg-emerald-50/60 px-3 py-2"><p className="text-xs font-extrabold text-emerald-900">{section.label}</p><p className="mt-1 text-xs leading-5 text-emerald-950/75">{section.items.slice(0, 4).map((item: any) => item.label).join(" · ")}{section.items.length > 4 ? " …" : ""}</p></div>)}</div></> : <p className="mt-4 rounded-xl bg-amber-50 px-3 py-3 text-sm font-bold text-amber-900">لم تُحدد بنود نطاق هذا المشروع بعد. اختر بنود التصميم والإشراف المطلوبة من مكتبة النطاق الشاملة.</p>}</article>
          </div>
