              ...
              {gate.gates.map((item: any, index: number) => {
                ...
                return (
                  <article key={item.id} className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                    ...
                    <button onClick={() => navigate(item.href)} className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-cyan-700 hover:text-cyan-900"><ExternalLink className="w-3.5 h-3.5" /> المصدر: {item.sourceLabel}</button>
                  </article>
                );
              })}
            </div>
