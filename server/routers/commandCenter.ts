...
export const commandCenterRouter = router({
  ...
  createSalwaLiveAvatarLiteSession: publicProcedure
    .input(z.object({
      token: z.string(),
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        const session = await createSalwaLiveAvatarLiteSession(ENV.liveAvatarApiKey, {
          isSandbox: input.isSandbox,
          avatarId: SALWA_LIVE_AVATAR_ID,
        });
        return session;
      } catch (error) {
        console.error("[LiveAvatar] LITE session creation failed", error);
        throw new TRPCError({ code: "BAD_GATEWAY", message: "تعذر تهيئة جلسة سلوى الحية الآمنة" });
      }
    }),
...
- كوني مختصرة وواضحة
- استخدمي اللغة العربية الفصحى فقط مع جميع الأعضاء، حتى إذا كان السؤال بلغة أخرى، ولا تكتبي جملة إنجليزية إلا إذا كان اسمًا رسميًا لا بديل عربيًا له
...
- إذا طلب العضو شيئاً خارج صلاحياتك، وجهيه بأدب`;
  ...
  // ─── Supervision Scope Report ───────────────────────────────────────────
  getSupervisionScopeReport: publicProcedure
    .input(z.object({ token: z.string(), projectId: z.number() }))
    .query(async ({ input }) => {
