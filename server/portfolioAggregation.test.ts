...
describe("Consolidated report source trace", () => {
  ...
  it("keeps every consolidated report connected to the shared clickable source-trace dialog", () => {
    ...
    expect(unifiedSource).toContain('data-testid="unified-executive-summary-row"');
    expect(unifiedSource).toContain("مدفوع حتى الآن");
    expect(unifiedSource).toContain("المتبقي حتى الذروة");
    expect(unifiedSource).toContain("رأس المال عند الذروة");
    expect(unifiedSource).toContain("شهر الذروة");
    expect(unifiedSource).toContain("تراكمي التمويل الجديد");
    expect(unifiedSource).not.toContain("ماذا سيحدث خلال الأشهر القادمة؟");
    expect(unifiedSource).not.toContain("دورة الأموال الكاملة");
    expect(unifiedSource).not.toContain("أين يتركز رأس المال عند ذروة المجموعة؟");
    expect(unifiedSource).not.toContain("لماذا هذه هي الذروة؟");
    expect(unifiedSource).not.toContain("تكاليف تطوير مؤكدة فقط");
    expect(unifiedSource.indexOf('data-testid="unified-executive-summary-row"')).toBeLessThan(unifiedSource.indexOf("الحركة الشهرية الموحّدة"));
