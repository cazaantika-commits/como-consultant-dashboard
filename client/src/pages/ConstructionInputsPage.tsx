...
export default function ConstructionInputsPage({ embedded }: { embedded?: boolean } = {}) {
  ...
  const pageCount = Math.max(1, Math.ceil(constructionMonths / CONSTRUCTION_MONTHS_PER_PAGE));
  const pageStart = calendarPage * CONSTRUCTION_MONTHS_PER_PAGE;
  const visibleMonths = monthlyProgress.slice(pageStart, pageStart + CONSTRUCTION_MONTHS_PER_PAGE);
    ...
    <div className="mx-auto max-w-[1440px]">
      ...
      {selectedProjectId && project && <div className="space-y-4">
        ...
        {constructionCost > 0 ? <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_290px]">
          <div className="fs-card fs-card-blue p-4">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 text-base font-extrabold text-slate-900"><CalendarDays className="h-5 w-5 text-teal-700" />تقويم التنفيذ الشهري</h2><p className="mt-1 text-xs text-slate-600">أدخل نسبة إنجاز كل شهر مباشرة. تتحرك قيمة العقد ومؤشر التقدم فورًا.</p></div><div className="flex items-center gap-2"><Button variant="outline" size="icon" className="h-8 w-8 border-slate-300" disabled={calendarPage === 0} onClick={() => setCalendarPage(p => p - 1)}><ChevronRight className="h-4 w-4" /></Button><span className="min-w-28 text-center text-sm font-bold text-slate-800">الأشهر {pageStart + 1}–{Math.min(pageStart + CONSTRUCTION_MONTHS_PER_PAGE, constructionMonths)}</span><Button variant="outline" size="icon" className="h-8 w-8 border-slate-300" disabled={calendarPage >= pageCount - 1} onClick={() => setCalendarPage(p => p + 1)}><ChevronLeft className="h-4 w-4" /></Button></div></div>
            <div className="grid gap-1.5 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">{visibleMonths.map((pct, localIndex) => { const index = pageStart + localIndex; const allocation = constructionCost * pct / 100; return <article key={index} className="fs-card fs-card-cyan p-2"><div className="mb-1 flex items-center justify-between"><span className="text-xs font-extrabold text-slate-900">شهر الإنشاء {index + 1}</span><span className="fs-pill fs-pill-teal text-[9px]">{pct}%</span></div><input aria-label={`إنجاز شهر الإنشاء ${index + 1}`} type="number" min={0} max={30} step={0.5} value={pct} onChange={e => handleMonthEdit(index, Number(e.target.value))} className="h-9 w-full rounded-lg border border-slate-400 bg-white text-center text-lg font-extrabold text-slate-950 outline-none ring-teal-200 focus:ring-2" /><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-gradient-to-l from-teal-600 to-cyan-300 transition-all" style={{ width: `${Math.min(100, pct / 30 * 100)}%` }} /></div><div className="mt-1 text-[10px] font-semibold text-slate-700">قيمة الأعمال: {formatFullNumber(allocation, "—")}</div></article>; })}</div>
          </div>
