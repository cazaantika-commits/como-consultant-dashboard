...
export const projects = mysqlTable("projects", {
	...
	surveyorDwgFees: decimal({ precision: 14, scale: 2 }),
	...
	designFeeFixed: decimal({ precision: 14, scale: 2 }),
	supervisionFeePct: decimal({ precision: 5, scale: 2 }),
	supervisionFeeFixed: decimal({ precision: 14, scale: 2 }),
	...
	residential1brCount: int().default(0),
	residential2brCount: int().default(0),
	residential2brMaidCount: int().default(0),
	residential3brCount: int().default(0),
	residential3brMaidCount: int().default(0),
	villaCount: int().default(0),
	townhouseCount: int().default(0),
	retailSmallCount: int().default(0),
	retailMediumCount: int().default(0),
	retailLargeCount: int().default(0),
	officeSmallCount: int().default(0),
	...
	studioArea: int().default(0),
	residential1brArea: int().default(750),
	residential2brArea: int().default(1300),
	residential2brMaidArea: int().default(0),
	residential3brArea: int().default(1650),
	residential3brMaidArea: int().default(0),
	villaArea: int().default(0),
	townhouseArea: int().default(0),
	...
	studioPrice: int().default(0),
	residential1brPrice: int().default(1550),
	residential2brPrice: int().default(1500),
	residential2brMaidPrice: int().default(0),
	residential3brPrice: int().default(1450),
	residential3brMaidPrice: int().default(0),
	villaPrice: int().default(0),
	townhousePrice: int().default(0),
...
export const cpaScopeItems = mysqlTable("cpa_scope_items", {
  ...
  label: varchar({ length: 200 }).notNull(),
  sectionId: int(),
  defaultType: mysqlEnum("cpa_si_defaultType", ['CORE', 'GREEN', 'RED', 'CONTRACTOR', 'SPECIALIST']).notNull().default('CORE'),
  description: text(),
  sortOrder: int().notNull().default(0),
  isActive: tinyint().notNull().default(1),
  createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
...
export const cpaScopeCategoryMatrix = mysqlTable("cpa_scope_category_matrix", {
  ...
  status: mysqlEnum("cpa_scm_status", ['INCLUDED', 'GREEN', 'RED', 'CONTRACTOR', 'NOT_REQUIRED', 'SPECIALIST']).notNull(),
  createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow().onUpdateNow().notNull(),
},
(table) => [
...
export const cpaProjects = mysqlTable("cpa_projects", {
  ...
  projectType: mysqlEnum('project_type', ['RESIDENTIAL', 'COMMERCIAL', 'MIXED_USE', 'OTHER']).default('RESIDENTIAL'),
  description: varchar('description', { length: 500 }),
  buaSqft: decimal('bua_sqft', { precision: 12, scale: 2 }).notNull(),
  buildingCategoryId: int('building_category_id'),
  constructionCostPerSqft: decimal('construction_cost_per_sqft', { precision: 10, scale: 2 }).notNull(),
...
export const comoNextEmailMessages = mysqlTable("como_next_email_messages", {
  id: bigint("id", { mode: "number" }).autoincrement().primaryKey(),
  userId: int("user_id").notNull().references(() => users.id, { onDelete: "restrict" }),
  mailboxKey: varchar("mailbox_key", { length: 64 }).notNull(),
  folderName: varchar("folder_name", { length: 255 }).notNull().default("INBOX"),
  uidValidity: varchar("uid_validity", { length: 64 }).notNull(),
  imapUid: bigint("imap_uid", { mode: "number" }).notNull(),
  messageId: varchar("message_id", { length: 1000 }),
  messageIdSha256: varchar("message_id_sha256", { length: 64 }).notNull(),
  ...
  serverSeen: tinyint("server_seen").notNull().default(0),
  attachmentCount: int("attachment_count").notNull().default(0),
  inboxStatus: mysqlEnum("inbox_status", ["unmatched", "suggested", "linked", "dismissed"]).notNull().default("unmatched"),
  importance: mysqlEnum("importance", ["unreviewed", "normal", "important", "urgent"]).notNull().default("unreviewed"),
  suggestedProjectId: int("suggested_project_id").references(() => projects.id, { onDelete: "restrict" }),
  suggestedWorkFileId: int("suggested_work_file_id"),
  suggestedProjectPartyId: int("suggested_project_party_id"),
  suggestionReason: text("suggestion_reason"),
  linkedProjectId: int("linked_project_id").references(() => projects.id, { onDelete: "restrict" }),
  linkedWorkFileId: int("linked_work_file_id"),
  linkedProjectPartyId: int("linked_project_party_id"),
  communicationId: bigint("communication_id", { mode: "number" }).references(() => comoNextCommunications.id, { onDelete: "restrict" }),
  replyDraftCommunicationId: bigint("reply_draft_communication_id", { mode: "number" }).references(() => comoNextCommunications.id, { onDelete: "restrict" }),
  ...
  uniqueIndex("como_next_email_mailbox_uid_uq").on(table.mailboxKey, table.folderName, table.uidValidity, table.imapUid),
  uniqueIndex("como_next_email_mailbox_message_uq").on(table.mailboxKey, table.messageIdSha256),
  index("como_next_email_user_status_idx").on(table.userId, table.inboxStatus, table.receivedAt),
  index("como_next_email_suggestion_idx").on(table.suggestedProjectId, table.suggestedWorkFileId),
  index("como_next_email_link_idx").on(table.linkedProjectId, table.linkedWorkFileId, table.receivedAt),
  foreignKey({ name: "como_next_email_suggested_file_fk", columns: [table.suggestedProjectId, table.suggestedWorkFileId], foreignColumns: [comoNextWorkFiles.projectId, comoNextWorkFiles.id] }).onDelete("restrict"),
  foreignKey({ name: "como_next_email_suggested_party_fk", columns: [table.suggestedProjectId, table.suggestedProjectPartyId], foreignColumns: [comoNextProjectParties.projectId, comoNextProjectParties.id] }).onDelete("restrict"),
  foreignKey({ name: "como_next_email_linked_file_fk", columns: [table.linkedProjectId, table.linkedWorkFileId], foreignColumns: [comoNextWorkFiles.projectId, comoNextWorkFiles.id] }).onDelete("restrict"),
  foreignKey({ name: "como_next_email_linked_party_fk", columns: [table.linkedProjectId, table.linkedProjectPartyId], foreignColumns: [comoNextProjectParties.projectId, comoNextProjectParties.id] }).onDelete("restrict"),
]);

export const comoNextEmailAttachments = mysqlTable("como_next_email_attachments", {
  id: bigint("id", { mode: "number" }).autoincrement().primaryKey(),
  emailMessageId: bigint("email_message_id", { mode: "number" }).notNull().references(() => comoNextEmailMessages.id, { onDelete: "cascade" }),
  ordinal: int("ordinal").notNull(),
  fileName: varchar("file_name", { length: 1000 }).notNull(),
  mimeType: varchar("mime_type", { length: 255 }).notNull(),
  byteSize: bigint("byte_size", { mode: "number" }).notNull().default(0),
  contentId: varchar("content_id", { length: 500 }),
  disposition: varchar("disposition", { length: 80 }),
  storageStatus: mysqlEnum("storage_status", ["metadata_only", "stored", "failed"]).notNull().default("metadata_only"),
  sha256: varchar("sha256", { length: 64 }),
  documentId: bigint("document_id", { mode: "number" }).references(() => comoNextDocuments.id, { onDelete: "restrict" }),
...
export const comoNextEmailAnalyses = mysqlTable("como_next_email_analyses", {
  ...
  emailMessageId: bigint("email_message_id", { mode: "number" }).notNull().references(() => comoNextEmailMessages.id, { onDelete: "cascade" }),
  requestKey: varchar("request_key", { length: 128 }).notNull(),
  analysisStatus: mysqlEnum("analysis_status", ["draft", "superseded"]).notNull().default("draft"),
  summaryAr: longtext("summary_ar").notNull(),
  importance: mysqlEnum("importance", ["normal", "important", "urgent"]).notNull().default("normal"),
  whyImportant: text("why_important"),
  suggestedNextStep: text("suggested_next_step"),
  replyDraftText: longtext("reply_draft_text"),
