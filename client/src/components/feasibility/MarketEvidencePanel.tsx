...
import DldCsvImportPanel from "./DldCsvImportPanel";
...
export default function MarketEvidencePanel({ projectId, decisionSnapshot }: { projectId: number; decisionSnapshot: Record<string, unknown> }) {
  ...
  const profile = evidenceQuery.data?.profile as any;
	const decisionState = evidenceQuery.data?.decisionState;
  const verifiedCount = evidence.filter((item: any) => item.verificationStatus === "verified").length;
  const reportLinksQuery = trpc.marketEvidence.getMarketReportLinks.useQuery({ projectId }, { enabled: !!projectId });
  const candidateReportsQuery = trpc.marketReports.getReports.useQuery({ status: "ready", limit: 20 }, { enabled: !!profile?.primaryCommunity });
  const handoffStatusQuery = trpc.marketEvidence.getPricingHandoffStatus.useQuery({ projectId }, { enabled: !!projectId });

	const refresh = () => { utils.marketEvidence.getProjectEvidence.invalidate({ projectId }); utils.marketEvidence.getSearchProfile.invalidate({ projectId }); utils.marketEvidence.getMarketReportLinks.invalidate({ projectId }); utils.marketEvidence.getPricingHandoffStatus.invalidate({ projectId }); utils.projectLaunchGate.get.invalidate({ projectId }); };
  ...
  const linkReport = trpc.marketEvidence.linkMarketReport.useMutation({ onSuccess: () => { toast.success("رُبط التقرير بفلترة السوق لهذا المشروع."); refresh(); }, onError: (error) => toast.error(error.message) });
  const unlinkReport = trpc.marketEvidence.unlinkMarketReport.useMutation({ onSuccess: refresh, onError: (error) => toast.error(error.message) });
  const handoffPricing = trpc.marketEvidence.handoffApprovedPricing.useMutation({ onSuccess: (result) => { toast.success(`تم تسليم ${result.fieldsUpdated.length} حقول سعر إلى التسعير فقط.`); setHandoffReady(false); refresh(); }, onError: (error) => toast.error(error.message) });
  ...
  const lastApproval = approvals[0] as any;
	const validApproval = decisionState?.isValid ? approvals.find((item: any) => item.id === decisionState.latestApproved?.id) as any : null;
	const decisionStatusLabel = decisionState?.status === "current_valid" ? "قرار ساري" : decisionState?.status === "needs_reapproval" ? "يتطلب إعادة اعتماد" : decisionState?.status === "incompatible_verified_evidence" ? "دليل غير متوافق" : decisionState?.status === "no_eligible_evidence" ? "بانتظار دليل موثق" : decisionState?.status === "no_profile" ? "بانتظار الفلترة" : "بانتظار الاعتماد";
  ...
  return (
    <Card className="border-violet-200 bg-gradient-to-l from-violet-50/45 via-white to-white shadow-sm">
      <CardContent className="p-5">
        ...
        {profile && <DldCsvImportPanel projectId={projectId} onImported={refresh} />}
        ...
        {profile && <div className="mt-4 rounded-xl border border-indigo-200 bg-indigo-50/55 p-4">
          <div className="flex items-start gap-2.5"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-indigo-700 text-white"><FilePlus2 className="h-4 w-4" /></div><div><h4 className="text-sm font-bold text-slate-900">تقارير السوق المرتبطة</h4><p className="mt-1 text-xs leading-5 text-slate-600">يرتبط التقرير بالمجتمع وسوق المقارنة الحاليين كمرجع قابل للمراجعة؛ لا يتحول تلقائيًا إلى سعر أو معاملة مقارنة.</p></div></div>
          <div className="mt-3 space-y-2">{(reportLinksQuery.data || []).map((item: any) => <div key={item.link.id} className="flex flex-col justify-between gap-2 rounded-lg border border-indigo-100 bg-white px-3 py-2.5 sm:flex-row sm:items-center"><div><p className="text-xs font-bold text-slate-900">{item.report.reportTitle}</p><p className="mt-0.5 text-[11px] text-slate-500">{item.report.source} · {item.report.community || "نطاق عام"} · {item.report.reportDate || "دون تاريخ"}</p>{item.link.relevanceNote && <p className="mt-1 text-[11px] text-indigo-800">{item.link.relevanceNote}</p>}</div><Button size="sm" variant="ghost" className="h-7 text-xs text-rose-700 hover:bg-rose-50 hover:text-rose-800" onClick={() => unlinkReport.mutate({ projectId, linkId: item.link.id })}>إلغاء الربط</Button></div>)}</div>
          <div className="mt-3 border-t border-indigo-100 pt-3"><p className="text-xs font-bold text-slate-700">تقارير جاهزة من مكتبة السوق لنطاق «{profile.primaryCommunity}»</p>{candidateReportsQuery.isLoading ? <p className="mt-2 text-xs text-slate-500">يجري فحص مكتبة التقارير…</p> : (candidateReportsQuery.data?.reports || []).filter((report: any) => !(reportLinksQuery.data || []).some((item: any) => item.report.id === report.id)).length === 0 ? <p className="mt-2 text-xs text-slate-500">لا يوجد تقرير جاهز مناسب بعد. ارفع التقرير من تبويب «تقارير السوق» ثم عُد لربطه هنا.</p> : <div className="mt-2 space-y-2">{(candidateReportsQuery.data?.reports || []).filter((report: any) => !(reportLinksQuery.data || []).some((item: any) => item.report.id === report.id)).map((report: any) => <div key={report.id} className="flex items-center justify-between gap-3 rounded-lg bg-white px-3 py-2 ring-1 ring-indigo-100"><span className="min-w-0 truncate text-xs font-medium text-slate-800">{report.reportTitle}</span><Button size="sm" variant="outline" className="h-7 shrink-0 border-indigo-200 text-xs text-indigo-800" disabled={linkReport.isPending} onClick={() => linkReport.mutate({ projectId, reportId: report.id, relevanceNote: `مرجع سوقي لنطاق ${profile.primaryCommunity}` })}>ربط بالتصفية</Button></div>)}</div>}</div>
        </div>}

        <div className="mt-4 rounded-xl border border-teal-200 bg-teal-50/55 p-4">
          <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center"><div><div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-teal-700" /><h4 className="text-sm font-bold text-slate-900">مراجعة واعتماد القرار</h4></div><p className="mt-1 text-xs leading-5 text-slate-600">يحفظ الاعتماد لقطة من المسودة والأدلة الموثقة. بعد ذلك فقط يظهر تسليم منفصل إلى التسعير؛ لا يوجد تسليم مباشر للتدفقات.</p>{lastApproval && <p className="mt-1.5 text-xs font-medium text-teal-800">آخر سجل: {lastApproval.decisionStatus === "approved" ? "معتمد" : lastApproval.decisionStatus === "reviewed" ? "تمت مراجعته" : "مرفوض"} · {new Date(lastApproval.decidedAt).toLocaleDateString("en-GB")}</p>}</div><div className="flex shrink-0 gap-2"><Button variant="outline" size="sm" disabled={recordDecision.isPending} onClick={() => recordDecision.mutate({ projectId, decisionStatus: "reviewed", recommendation: { product: (decisionSnapshot as any).product || {}, pricing: (decisionSnapshot as any).pricing || {} }, notes })}>تسجيل مراجعة</Button><Button size="sm" disabled={recordDecision.isPending || verifiedCount === 0 || decisionState?.status === "incompatible_verified_evidence"} onClick={() => recordDecision.mutate({ projectId, decisionStatus: "approved", recommendation: { product: (decisionSnapshot as any).product || {}, pricing: (decisionSnapshot as any).pricing || {} }, notes })} className="bg-teal-700 hover:bg-teal-800">اعتماد المسودة</Button></div></div><div className={`mt-3 rounded-xl border px-3 py-2.5 ${decisionState?.isValid ? "border-emerald-200 bg-emerald-50" : decisionState?.status === "needs_reapproval" || decisionState?.status === "incompatible_verified_evidence" ? "border-rose-200 bg-rose-50" : "border-amber-200 bg-amber-50"}`}><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-bold text-slate-900">{decisionStatusLabel}{decisionState?.latestApproved?.id ? ` · قرار #${decisionState.latestApproved.id}` : ""}</p>{decisionState?.profile && <span className="rounded-full bg-white px-2 py-1 text-[10px] font-bold text-slate-600 ring-1 ring-slate-200">فلترة v{decisionState.profile.version} · {String(decisionState.profile.hash || "").slice(0, 8)}</span>}</div><p className="mt-1 text-[11px] leading-5 text-slate-700">{decisionState?.reason || "يجري فحص صلاحية القرار."}</p><p className="mt-1 text-[11px] text-slate-500">الخادم يثبت كامل الأدلة الموثقة مع بصمتها؛ لا يمكن اختيار دليل انتقائي من المتصفح.</p></div><input value={notes} onChange={(event) => setNotes(event.target.value)} className="field mt-3 bg-white" placeholder="ملاحظة اختيارية تفسر المراجعة أو الاعتماد" />{verifiedCount === 0 && <p className="mt-2 text-xs text-amber-800">يلزم توثيق دليل واحد على الأقل قبل تفعيل «اعتماد المسودة».</p>}
          {validApproval && <div className="mt-3 rounded-lg border border-emerald-200 bg-white p-3"><div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center"><div><p className="text-xs font-bold text-slate-900">تسليم القرار الساري إلى صفحة التسعير</p><p className="mt-1 text-[11px] leading-4 text-slate-600">ينقل فقط الأسعار الموجودة في القرار الساري وبإجراء صريح مستقل. أي تغير في الفلترة أو الأدلة يغلق التسليم تلقائيًا.</p>{handoffStatusQuery.data && <p className="mt-1 text-[11px] font-medium text-emerald-800">آخر تسليم: {new Date(handoffStatusQuery.data.handedOffAt).toLocaleDateString("en-GB")}</p>}</div><Button size="sm" className="shrink-0 bg-emerald-700 hover:bg-emerald-800" onClick={() => setHandoffReady((value) => !value)}>تجهيز التسليم</Button></div>{handoffReady && <div className="mt-3 flex flex-col gap-2 border-t border-emerald-100 pt-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs font-medium text-emerald-900">هل تؤكد نقل الأسعار فقط من القرار الساري #{validApproval.id}؟</p><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => setHandoffReady(false)}>إلغاء</Button><Button size="sm" disabled={handoffPricing.isPending} className="bg-emerald-700 hover:bg-emerald-800" onClick={() => handoffPricing.mutate({ projectId, approvalId: validApproval.id })}>تأكيد التسليم إلى التسعير</Button></div></div>}</div>}
        </div>
      </CardContent>
    </Card>
  );
