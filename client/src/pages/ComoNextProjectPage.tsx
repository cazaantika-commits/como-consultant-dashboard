  ...
  return <div dir="rtl" className="min-h-screen bg-[radial-gradient(circle_at_top_right,#fff8e9_0,#f8faf9_34%,#eef3f2_100%)] text-slate-900">
    <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/88 backdrop-blur-xl">
      <div className="mx-auto flex min-h-16 max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-7">
        ...
        <div className="flex flex-wrap items-center gap-2"><Button variant="outline" onClick={() => navigate(`/project-management?projectId=${projectId}`)} className="rounded-xl bg-white text-xs"><BriefcaseBusiness className="ml-1 h-3.5 w-3.5" />إدارة المشروع</Button><Button variant="outline" onClick={() => navigate(`/project/${projectId}`)} className="rounded-xl bg-white text-xs">بطاقة المشروع الأصلية</Button><Button variant="outline" onClick={() => navigate(`/project-launch/${projectId}`)} className="rounded-xl bg-white text-xs"><FileCheck2 className="ml-1 h-3.5 w-3.5" />بوابة تأسيس المشروع</Button><Button variant="outline" onClick={() => navigate(`/development-phases?projectId=${projectId}`)} className="rounded-xl bg-white text-xs"><Route className="ml-1 h-3.5 w-3.5" />جولة مراحل التطوير</Button><Button variant="outline" onClick={() => navigate(`/bateekha?projectId=${projectId}`)} className="rounded-xl bg-white text-xs">الدراسات المحمية</Button></div>
      </div>
    </header>
