      ...
      <section className="relative overflow-hidden rounded-[38px] bg-[#102d36] text-white shadow-[0_30px_90px_rgba(15,36,45,.20)]">
        ...
        <div className="relative grid gap-8 px-6 py-8 sm:px-10 lg:grid-cols-[1.18fr_.82fr] lg:px-12 lg:py-11">
          ...
          <div className="self-end rounded-[28px] border border-white/10 bg-white/7 p-5 backdrop-blur"><div className="flex items-center justify-between gap-3"><div><p className="text-xs text-slate-300">البوابات المكتملة</p><p className="mt-1 text-4xl font-black"><bdi>{gate.completeGateCount}</bdi><span className="text-lg text-slate-400"> / {gate.totalGateCount}</span></p></div><ClipboardCheck className="h-10 w-10 text-amber-200" /></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-amber-300" style={{ width: `${Math.round((gate.completeGateCount / gate.totalGateCount) * 100)}%` }} /></div><p className="mt-4 text-xs leading-6 text-slate-300">جاهزية طلب العروض: <strong className={gate.readyForTender ? "text-emerald-300" : "text-amber-200"}>{gate.readyForTender ? "نعم — وفق المصادر الحالية" : "ليست مكتملة بعد"}</strong></p></div>
			...
			<section className={`mt-5 rounded-[26px] border p-5 ${gate.marketDecision.isValid ? "border-emerald-200 bg-emerald-50/80" : gate.marketDecision.status === "needs_reapproval" || gate.marketDecision.status === "incompatible_verified_evidence" ? "border-rose-200 bg-rose-50/80" : "border-amber-200 bg-amber-50/75"}`}><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[10px] font-black text-slate-500">قرار السوق والاستثمار · قراءة من المصدر</p><h2 className="mt-1 text-lg font-black text-slate-950">{gate.marketDecision.isValid ? "قرار ساري" : gate.marketDecision.status === "needs_reapproval" ? "يتطلب إعادة اعتماد" : "غير جاهز للاعتماد"}{gate.marketDecision.decisionId ? ` · #${gate.marketDecision.decisionId}` : ""}</h2><p className="mt-2 max-w-3xl text-xs leading-6 text-slate-700">{gate.marketDecision.reason}</p></div><div className="rounded-2xl bg-white px-4 py-3 text-xs text-slate-600 shadow-sm"><p className="font-bold text-slate-900">فلترة v{gate.marketDecision.profileVersion || "—"}</p><p className="mt-1">{gate.marketDecision.verifiedEvidenceCount} دليل موثق</p></div></div></section>
