...
export function ExecutiveFourMonthFocus({ variant, onClose }: FocusProps) {
  ...
  if (!isPanel) {
    return (
      <section className="overflow-hidden rounded-3xl border border-amber-200 bg-gradient-to-l from-amber-50 via-white to-indigo-50 p-5 shadow-sm" dir="rtl" data-testid="shared-executive-briefing">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-start gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-amber-100 text-amber-700"><Landmark className="h-5 w-5" /></span><div><p className="text-[11px] font-black text-amber-700">ملخص افتتاحي</p><h2 className="mt-1 text-lg font-black text-slate-950">المحفظة التنفيذية</h2><p className="mt-1 text-xs leading-5 text-slate-600">هذه هي القرارات والأرقام الأقرب قبل الدخول إلى التفاصيل.</p></div></div><span className="self-start rounded-full border border-amber-200 bg-white px-3 py-1 text-[10px] font-black text-amber-800">الأشهر الأربعة القادمة</span></div>
