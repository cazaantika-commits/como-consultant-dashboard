...
// Import schema types
const schema = {}; // Will be loaded at runtime
  ...
  try {
    ...
    for (const consultantName of ALL_CONSULTANTS_DATA) {
      try {
        ...
        console.log(`  ⚠️ ${consultantName} (قد يكون موجوداً بالفعل)`);
      }
    }
          ...
          for (const consultantName of projectData.consultantNames) {
            ...
            if (consultantId) {
              try {
                ...
                console.log(`     ⚠️ فشل ربط ${consultantName}`);
              }
            }
          }
        }
      } catch (error) {
        console.log(`  ⚠️ ${projectData.name} (قد يكون موجوداً بالفعل)`);
      }
    }
