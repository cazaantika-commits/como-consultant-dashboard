    ...
    <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-background" dir="rtl">
      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-9">
        ...
        {reference && <section className="mt-6 space-y-5">
          ...
          <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]"><section className="rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><FileText className="h-4 w-4 text-teal-700" /><h3 className="font-extrabold text-foreground">أحدث الوثائق المفهرسة</h3></div>{reference.driveFolder?.configured ? <Button variant="outline" size="sm" onClick={() => window.open(reference.driveFolder.url, "_blank", "noopener,noreferrer")} className="gap-1.5 text-xs"><ExternalLink className="h-3.5 w-3.5" />افتح مجلد Drive الأصلي</Button> : <span className="text-xs font-bold text-amber-700">مجلد Drive غير موثق بعد</span>}</div><p className="mt-2 text-xs leading-5 text-muted-foreground">الفهرس مرجع للملف الأصلي فقط؛ لا ينشئ نسخة ثانية من الملفات.</p>{reference.officialDocuments.length > 0 ? <div className="mt-4 space-y-2">{reference.officialDocuments.map((document: any) => <div key={`${document.sourceName}-${document.updatedAt}`} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-3"><div><p className="text-sm font-bold text-foreground">{document.sourceName}</p><p className="mt-1 text-xs text-muted-foreground">{document.category || "وثيقة مرجعية"} · آخر فهرسة: {document.updatedAt || "غير محدد"}</p></div>{document.driveUrl ? <button onClick={() => window.open(document.driveUrl, "_blank", "noopener,noreferrer")} className="inline-flex items-center gap-1 text-xs font-bold text-teal-700 hover:text-teal-900"><ExternalLink className="h-3.5 w-3.5" />افتح الأصل</button> : null}</div>)}</div> : <p className="mt-4 rounded-xl border border-dashed border-slate-200 p-4 text-xs leading-5 text-muted-foreground">لا يوجد مستند مفهرس لهذا المشروع بعد. يبقى Google Drive هو مكان الملف الأصلي.</p>}</section>
            <section className="rounded-2xl border border-amber-200 bg-amber-50/50 p-5 shadow-sm"><p className="text-xs font-bold text-amber-800">مرحلة لاحقة عند بدء التنفيذ</p><h3 className="mt-1 font-extrabold text-foreground">{reference.changeControl.title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{reference.changeControl.detail}</p><div className="mt-4 rounded-xl border border-amber-200 bg-white/70 px-3 py-3 text-xs font-bold text-amber-900">{reference.changeControl.activationRule}</div><div className="mt-4 space-y-2"><p className="text-xs font-bold text-foreground">محتوى الطلب عند تفعيله:</p>{reference.changeControl.template.map((item: string) => <div key={item} className="rounded-lg border border-amber-100 bg-white/70 px-3 py-2 text-xs text-foreground">{item}</div>)}</div></section>
          </div>
        </section>}
      </main>
    </div>
