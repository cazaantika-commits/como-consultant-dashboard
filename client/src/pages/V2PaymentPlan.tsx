    ...
    <main dir="rtl" className="min-h-screen w-full max-w-full overflow-x-hidden bg-slate-50 px-3 py-4 text-slate-950 sm:px-5 lg:px-8">
      <div className="mx-auto w-full max-w-[1280px] space-y-4">
        ...
        <details className="rounded-2xl border-2 border-slate-300 bg-white shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4"><span><span className="flex items-center gap-2 font-black text-slate-950"><SlidersHorizontal className="h-4 w-4 text-indigo-700" />تعديل يدوي لدفعة موجودة</span><span className="mt-1 block text-xs text-slate-600">اختياري؛ استخدمه فقط عند الحاجة لتعديل دفعة أُنشئت بالفعل.</span></span><ChevronDown className="h-5 w-5 text-slate-500" /></summary><div className="border-t-2 border-slate-200 p-5">
          ...
          <details className="mt-4 rounded-xl border border-sky-200 bg-sky-50/60 p-4"><summary className="cursor-pointer text-sm font-black text-sky-950">إنشاء دفعات إنشاء كل عدة أشهر</summary><div className="mt-3 grid gap-3 sm:grid-cols-4"><label className="text-xs font-bold text-slate-700">إجمالي النسبة<input type="number" min={0} max={100} value={seriesTotal} onChange={(event) => setSeriesTotal(Math.max(0, Number(event.target.value) || 0))} className="mt-1 h-9 w-full rounded border border-slate-400 bg-white px-2 text-center font-black" /></label><label className="text-xs font-bold text-slate-700">تبدأ بعد (شهر)<input type="number" min={1} value={seriesFirstAfter} onChange={(event) => setSeriesFirstAfter(Math.max(1, Number(event.target.value) || 1))} className="mt-1 h-9 w-full rounded border border-slate-400 bg-white px-2 text-center font-black" /></label><label className="text-xs font-bold text-slate-700">كل كم شهر<input type="number" min={1} value={seriesEvery} onChange={(event) => setSeriesEvery(Math.max(1, Number(event.target.value) || 1))} className="mt-1 h-9 w-full rounded border border-slate-400 bg-white px-2 text-center font-black" /></label><Button onClick={addPeriodicSeries} className="self-end bg-sky-700 text-white hover:bg-sky-600">إنشاء الدفعات</Button></div></details>
        </div></details>
      </div>
    </main>
  );
