    ...
    <main className="mx-auto max-w-7xl px-4 py-7 sm:px-7 sm:py-10">
      ...
      {!selectedProjectId ? <Card className="mt-6 rounded-[30px] border-dashed border-slate-300 bg-white/75 p-9 text-center"><LayoutDashboard className="mx-auto h-9 w-9 text-slate-400" /><h2 className="mt-4 text-xl font-black text-slate-900">اختر مشروعًا للبدء</h2><p className="mt-2 text-sm leading-7 text-slate-500">بعد الاختيار ستظهر بيانات الأرض والمساحات، ثم البطاقات الأربع الخاصة بالمشروع نفسه.</p></Card> : projectQuery.isLoading ? <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[1,2,3,4].map(item => <Skeleton key={item} className="h-48 rounded-[28px]" />)}</div> : projectQuery.isError || !project ? <Card className="mt-6 rounded-[30px] border-rose-200 bg-rose-50 p-8 text-center"><CircleAlert className="mx-auto h-8 w-8 text-rose-600" /><h2 className="mt-4 text-xl font-black text-rose-950">تعذر فتح المشروع المختار</h2><p className="mt-2 text-sm text-rose-700">{projectQuery.error?.message || "لا تتوفر صلاحية المشروع."}</p></Card> : <>
        <section className="mt-6 overflow-hidden rounded-[30px] border border-slate-200 bg-white/90 shadow-sm">
					<div className="grid gap-5 p-5 sm:p-6 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="text-[11px] font-black text-[#1f6478]">بطاقة التعريف الحالية</p><h2 className="mt-2 text-2xl font-black text-slate-950">{project.name}</h2><p className="mt-2 max-w-3xl text-sm leading-7 text-slate-500">{project.description || "المشروع الرسمي المختار. البيانات أدناه تُقرأ من بطاقة المشروع الحالية."}</p>{foundation?.marketDecision ? <button type="button" onClick={() => navigate(`/knowledge-analysis?projectId=${selectedProjectId}`)} className={`mt-4 flex w-full max-w-2xl flex-wrap items-center justify-between gap-3 rounded-2xl border px-4 py-3 text-right ${foundation.marketDecision.isValid ? "border-emerald-200 bg-emerald-50" : foundation.marketDecision.status === "needs_reapproval" || foundation.marketDecision.status === "incompatible_verified_evidence" ? "border-rose-200 bg-rose-50" : "border-amber-200 bg-amber-50"}`}><span><span className="block text-[10px] font-black text-slate-500">قرار السوق والاستثمار · قراءة فقط</span><span className="mt-1 block text-sm font-black text-slate-900">{foundation.marketDecision.isValid ? `قرار ساري #${foundation.marketDecision.decisionId}` : foundation.marketDecision.status === "needs_reapproval" ? "يتطلب إعادة اعتماد" : "غير جاهز"}</span><span className="mt-1 block text-[11px] leading-5 text-slate-600">{foundation.marketDecision.reason}</span></span><ChevronLeft className="h-4 w-4 shrink-0 text-slate-500" /></button> : null}</div>{foundation ? <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-left"><p className="text-[10px] font-black text-amber-800">محطة التأسيس</p><p className="mt-1 text-sm font-black text-slate-900"><bdi>{foundation.completeGateCount}</bdi> / <bdi>{foundation.totalGateCount}</bdi> مكتملة</p></div> : null}</div>
