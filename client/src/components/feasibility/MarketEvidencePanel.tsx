...
export default function MarketEvidencePanel({ projectId, decisionSnapshot }: { projectId: number; decisionSnapshot: Record<string, unknown> }) {
  ...
  const refresh = () => { utils.marketEvidence.getProjectEvidence.invalidate({ projectId }); utils.marketEvidence.getMarketReportLinks.invalidate({ projectId }); utils.marketEvidence.getPricingHandoffStatus.invalidate({ projectId }); };
    ...
    <Card className="border-violet-200 bg-gradient-to-l from-violet-50/45 via-white to-white shadow-sm">
      <CardContent className="p-5">
        ...
        <div className="mt-4 rounded-xl border border-teal-200 bg-teal-50/55 p-4">
          ...
          {lastApproval?.decisionStatus === "approved" && <div className="mt-3 rounded-lg border border-emerald-200 bg-white p-3"><div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center"><div><p className="text-xs font-bold text-slate-900">تسليم معتمد إلى صفحة التسعير</p><p className="mt-1 text-[11px] leading-4 text-slate-600">ينقل فقط الأسعار الموجودة في لقطة القرار المعتمدة إلى التسعير والإيرادات، مع حفظ سجل مستقل. لا يغيّر إعدادات التدفق أو جدول التوزيع.</p>{handoffStatusQuery.data && <p className="mt-1 text-[11px] font-medium text-emerald-800">آخر تسليم: {new Date(handoffStatusQuery.data.handedOffAt).toLocaleDateString("en-GB")}</p>}</div><Button size="sm" className="shrink-0 bg-emerald-700 hover:bg-emerald-800" onClick={() => setHandoffReady((value) => !value)}>تجهيز التسليم</Button></div>{handoffReady && <div className="mt-3 flex flex-col gap-2 border-t border-emerald-100 pt-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs font-medium text-emerald-900">هل تؤكد نقل الأسعار فقط من هذا القرار المعتمد؟</p><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => setHandoffReady(false)}>إلغاء</Button><Button size="sm" disabled={handoffPricing.isPending} className="bg-emerald-700 hover:bg-emerald-800" onClick={() => handoffPricing.mutate({ projectId, approvalId: lastApproval.id })}>تأكيد التسليم إلى التسعير</Button></div></div>}</div>}
        </div>
      </CardContent>
    </Card>
  );
