            ...
            <section className="grid items-start gap-3 xl:grid-cols-[minmax(0,1fr)_300px]">
              <div className="min-w-0 space-y-4">
                ...
                <section data-testid="pricing-source-panel" className="overflow-hidden rounded-[22px] border-2 border-amber-300 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.07)]">
                  ...
                  <div className="pricing-ledger-grid grid gap-2 p-3 lg:grid-cols-2">
                    {activeUnitRevenues.map((unit, index) => (
                      <article key={unit.id} data-testid="unit-pricing-card" className="pricing-ledger-row overflow-hidden rounded-xl border-r-[5px] border-slate-200 bg-white shadow-sm" style={{ borderRightColor: unit.color, background: `linear-gradient(100deg, ${unit.color}13 0%, #ffffff 45%)` }}>
                        <div className="flex min-h-[92px] items-center gap-2.5 p-2.5">
                          <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg text-sm font-black text-white shadow-sm" style={{ backgroundColor: unit.color }}>{String(index + 1).padStart(2, "0")}</span>
