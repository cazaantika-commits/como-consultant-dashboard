...
## 2026-08-18 — Wael workspace pilot: initial live rendering

The Financial Studies Planning guide now exposes one visible **المبيعات والتسويق — مساحة وائل** entry and no separate Marketing card. The first render attempt surfaced a `Cannot access 'timeline' before initialization` error caused by the new marketing-plan loading effect; it was corrected by deriving the project marketing timing locally during plan loading. After reload, the workspace opened successfully for **مجان متعدد الاستخدامات (G+4P+25)**, retaining the existing pricing, sales distribution, payment-plan, escrow, and cash-inflow outputs while presenting the embedded Marketing decision section immediately before Escrow.

The live review also confirms that the underlying workflow is still visually dense and exposes many raw unit, payment, and monthly schedule inputs. The protected pilot is functional but is not yet considered the final intelligent Wael experience; the next refinement must make decision controls and live outcomes primary while retaining detailed grids as secondary controls.

The subsequent decision-first layer was verified live. It presents Wael with sale-target, sales-pace, price-adjustment, and buyer-liquidity controls above the detailed grids, together with live scenario revenue, first-collection month, critical-escrow month, expected profit, and escrow-status outcomes. Selecting the non-persistent **سريع** sales-pace control immediately moved the critical escrow indication from month 11 to month 8, recalculated the monthly sales and cash-inflow schedule, and exposed the single **اعتماد سيناريو وائل** action. The browser was then reloaded without saving, discarding the temporary scenario and preserving the approved saved plan.

The final live pilot view now defaults to the **مركز قرار وائل** and its compact outcome cards. The prior pricing, payment-plan, schedule, detailed monthly sales, marketing, escrow, and collection grids remain available only through **تفاصيل التحكم الدقيق**. Opening that disclosure revealed the preserved existing controls and calculations; closing it returns Wael to the decision-first view. The current timing badge now correctly identifies marketing as **سيناريو وائل**, not a separate Marketing page.

After a fresh browser reload, the sole **المبيعات والتسويق — مساحة وائل** card remained reachable through **الإعداد والتخطيط**. It reopened with **مركز قرار وائل** visible first and the advanced controls collapsed, confirming that the decision-first presentation is the default rather than a temporary browser state.

For the confirmed controlled persistence test, the first unit price was changed from AED 1,600 to AED 1,601. The decision layer immediately updated scenario revenue from AED 725.7M to AED 725.8M and the detailed collection schedule, while retaining the same first-collection and critical-escrow months. The single **اعتماد سيناريو وائل** action then completed, proving that the consolidated workspace exposes one scenario-save path for the combined Sales and Marketing data. The original AED 1,600 value remains scheduled for immediate restoration after reload and downstream checks.

## 2026-08-18 — Command Center access repair and executive alert

The reported Command Center login failure was reproduced as a post-login dashboard crash rather than an invalid access token. A valid member token reached the server, but the dashboard attempted to render stale `BUBBLES[9]` through `BUBBLES[12]` entries after those legacy cards had already been removed. The resulting `undefined.color` error stopped the dashboard from rendering. The layout was corrected to use only the current nine dashboard cards.

After the repair, the authenticated Command Center loaded successfully for **عبدالرحمن زقوت**. The new read-only executive section **التزامات المحفظة القادمة** rendered above the Command Center content and displayed the next four real calendar months from the same Project Aggregation source: September 2026 AED 4.5M required, October AED 6.6M required, November AED 8.7M required, and December AED 10.6M required. The headline required amount was AED 30.5M and the received amount was AED 0. The section exposes 3-/4-month horizon controls, project-driver drill-downs, and a link to the full aggregation report. No financial calculation was added or changed.

The September card drill-down was opened successfully. It attributed the AED 4.5M required amount to Majan Mixed-Use AED 2.6M, Majan Commercial Center AED 1.8M, and Nad Al Sheba Plot 3 Villas AED 153K. Switching the same alert to the 3-month horizon removed December, recalculated the headline requirement to AED 19.9M, and retained the same September–November project-driven details. This confirms that both horizons are read-only views of the same live aggregation data.

The **فتح التقرير المجمّع الكامل** action was corrected to open `/bateekha?tab=portfolio` and verified live. It now loads the exact current **تجميع المشاريع** report directly, showing its six-project portfolio table and the same September AED 4.5M, October AED 6.6M, November AED 8.7M, and December AED 10.6M totals. It no longer stops at the Financial Studies landing guide.

## 2026-08-18 — Wael professional studio redesign

The redesigned **المبيعات والتسويق — مساحة وائل** was opened live for Majan. It now presents a dark professional studio header, a six-room decision rail (**لوحة السيناريو**; **المنتج والسعر**; **خطة البيع**; **تحصيل المشتري**; **حملة التسويق**; and **أثر القرار**), and a clean default Scenario Board. The opening board shows Wael’s 80% / 167-of-209-unit target, sales-pace choices, price actions, buyer-liquidity choices, AED 725.7M scenario revenue, first collection in month 8, critical month 11, AED 133.0M expected profit, and a balanced escrow status. The prior long collection of raw grids is no longer the default experience.

The dedicated **المنتج والسعر** room was opened and showed only the preserved unit-price contract and its revenue impact. The **خطة البيع** room was then opened and correctly isolated sales settings, timeline, and the synchronized units/percentage schedule. This exposed one remaining usability issue: the month-by-month precision editor still renders a dense 30-month grid as the default inside the Sales Plan room. It must be replaced by a goal-and-pace planning board, with the raw monthly editor reserved for an explicit precision-edit action.

After refinement, the live **خطة البيع** room now defaults to a goal-and-pace planner. It shows the project’s 80% sale target (167 of 209 units), four sales-pace choices, and a small proposed-distribution visualization. The 30-month synchronized units/percentage grid is no longer visible by default; Wael must deliberately choose **تخصيص الأشهر بدقة** to reveal it. The live page explicitly explains that Wael does not need to fill 30 cells and can instead adjust the target or pace.

The Sales Plan was then refined into a visible 12-month direct-input board. Its first page now shows April through March as editable month cards, each with synchronized **وحدات** and **النسبة %** inputs. A non-persistent April percentage edit immediately changed the unit value and the live plan total from 167 to 164 units, confirming that the card itself is the input surface rather than a read-only summary. Reloading without saving discarded the test and restored the approved plan.
