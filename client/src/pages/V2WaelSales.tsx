...
export default function V2WaelSales({ embedded }: { embedded?: boolean } = {}) {
  ...
  const [impactFocus, setImpactFocus] = useState("توزيع المبيعات");
    ...
    try {
      ...
      const savedPlan = await saveWorkspace.mutateAsync({
        ...
        salesAbsorptionJson: JSON.stringify({
          ...
          manual: manualUnits.map((value) => Math.max(0, Math.round(Number(value) || 0))),
          ppDownPct,
          ppSecondPct,
          ppSecondAfterMonths,
          ppInstallmentPct,
  ...
  const updateSalesMonth = (monthIndex: number, requestedUnits: number) => {
    const current = [...(manualUnits.length === salesMonths ? manualUnits : salesDistribution)].map((value) => Math.max(0, Math.round(Number(value) || 0)));
  ...
  const collectionPastelTones = [
    "border-sky-300 bg-sky-50/85 text-sky-900",
    "border-violet-300 bg-violet-50/85 text-violet-900",
    "border-amber-300 bg-amber-50/85 text-amber-950",
    "border-emerald-300 bg-emerald-50/85 text-emerald-950",
    "border-rose-300 bg-rose-50/85 text-rose-950",
    "border-indigo-300 bg-indigo-50/85 text-indigo-950",
    "border-teal-300 bg-teal-50/85 text-teal-950",
  ];
  const formatCalendarMonth = (projectMonth: number) => {
    if (!projectStartDate) return `شهر ${projectMonth}`;
    const [startYear, startMonth] = projectStartDate.split("-").map(Number);
    if (!startYear || !startMonth) return `شهر ${projectMonth}`;
    const monthNames = ["يناير", "فبراير", "مارس", "أبريل", "مايو", "يونيو", "يوليو", "أغسطس", "سبتمبر", "أكتوبر", "نوفمبر", "ديسمبر"];
    const offset = startMonth - 1 + projectMonth - 1;
    return `${monthNames[offset % 12]} ${startYear + Math.floor(offset / 12)}`;
  };
  // Retained only to keep the detailed legacy renderers dormant while the unified
  // canvas supplies the entire visible workflow. No room selector is rendered.
  const activeStudioRoom = "canvas";
  const activeRoom = { eyebrow: "", title: "", description: "", icon: LayoutDashboard, tone: "bg-slate-900 text-white" };

  // ═══════════════════════════════════════════════════════════════════════════════
  // RENDER
  // ═══════════════════════════════════════════════════════════════════════════════
  return (
    <div className="sales-workspace-pastel min-h-full bg-[#f5f7fb] p-3 sm:p-4" dir="rtl">
      <div className="mx-auto max-w-[1500px] space-y-4">
        {/* ═══ SCENARIO STUDIO HEADER ═══ */}
        <section className="overflow-hidden rounded-[24px] border border-teal-200 bg-white shadow-[0_16px_40px_rgba(15,23,42,0.08)]">
          <div className="flex flex-col gap-4 bg-[radial-gradient(circle_at_12%_0%,rgba(45,212,191,0.22),transparent_38%),linear-gradient(115deg,#f0fdfa,#eff6ff_56%,#f5f3ff)] px-5 py-5 text-slate-900 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-2xl">
              <div className="flex items-center gap-2 text-[11px] font-bold tracking-wide text-teal-700"><Sparkles className="h-4 w-4" />مساحة سيناريو وائل</div>
              <h1 className="mt-1 text-2xl font-black tracking-tight sm:text-[28px]">صمّم البيع، وشاهد السيولة قبل الاعتماد.</h1>
              <p className="mt-1.5 text-sm leading-6 text-slate-600">{isBuildForSale ? "تتحرك كل نتيجة هنا فور تعديل السعر أو شهر البيع المباشر بعد الإنجاز." : "تتحرك كل نتيجة هنا فور تعديل السعر أو الشهر أو خطة التحصيل."} السعر لكل قدم مربع يظهر في لوحة تسعير واحدة واضحة.</p>
            </div>
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              }} className="h-10 gap-1.5 border-teal-200 bg-white px-3 text-teal-800 hover:bg-teal-50 hover:text-teal-900">
          ...
          <>
            <section className="grid items-start gap-3 xl:grid-cols-[minmax(0,1fr)_300px]">
              <div className="min-w-0 space-y-4">
                <section data-testid="sales-control-strip" className="overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.07)]">
                  <div className="flex flex-col gap-2 border-b border-slate-100 bg-slate-50/80 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <p className="text-[11px] font-bold text-teal-700">شريط التحكم</p>
                      <h2 className="mt-0.5 text-lg font-black text-slate-900">حرّك المحرّك، والأثر أمامك فورًا</h2>
                    </div>
                  ...
                  <div className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div className="rounded-2xl border border-blue-100 bg-gradient-to-br from-blue-50 to-white p-4">
                      <div className="flex items-center justify-between"><p className="text-xs font-bold text-slate-700">إيقاع البيع</p><TrendingUp className="h-4 w-4 text-blue-700" /></div>
                      <div className="mt-3 grid grid-cols-4 gap-1.5">{([{ id: "fast", label: "سريع", speed: 85 }, { id: "bell", label: "متوازن", speed: 50 }, { id: "gradual", label: "تدريجي", speed: 30 }, { id: "late", label: "متأخر", speed: 15 }] as const).map((option) => <button key={option.id} type="button" onClick={() => { applySalesPace(option.id, option.speed); setImpactFocus("إيقاع البيع"); }} className={`rounded-xl border px-1 py-2 text-[10px] font-black transition ${curveTemplate === option.id && salesMode === "auto" ? "border-blue-300 bg-blue-100 text-blue-800 shadow-sm" : "border-slate-200 bg-white text-slate-600 hover:bg-blue-50"}`}>{option.label}</button>)}</div>
                      <p className="mt-3 text-[11px] text-slate-500">توزيع مقترح على {salesMonths} شهر</p>
                    </div>
                    <div className="rounded-2xl border border-amber-100 bg-gradient-to-br from-amber-50 to-white p-4">
                      <div className="flex items-center justify-between"><p className="text-xs font-bold text-slate-700">متوسط سعر الوحدة</p><DollarSign className="h-4 w-4 text-amber-700" /></div>
                    ...
                    {isBuildForSale ? <div className="rounded-2xl border border-indigo-100 bg-gradient-to-br from-indigo-50 to-white p-4"><div className="flex items-center justify-between"><p className="text-xs font-bold text-slate-700">تحصيل البيع</p><CreditCard className="h-4 w-4 text-indigo-700" /></div><p className="mt-3 text-lg font-black text-indigo-800">دفعة كاملة عند بيع الوحدة</p><p className="mt-2 text-[11px] leading-5 text-slate-500">تبدأ المبيعات بعد الإنجاز، وتدخل حصيلة كل وحدة كاملة مباشرة إلى حساب المستثمر.</p></div> : <div className="rounded-2xl border border-indigo-100 bg-gradient-to-br from-indigo-50 to-white p-4"><div className="flex items-center justify-between"><p className="text-xs font-bold text-slate-700">خطة تحصيل المشتري</p><CreditCard className="h-4 w-4 text-indigo-700" /></div><div className="mt-3 grid grid-cols-3 gap-1.5">{([{ id: "early", label: "مبكرة" }, { id: "balanced", label: "متوازنة" }, { id: "handover", label: "تسليم" }] as const).map((option) => <button key={option.id} type="button" onClick={() => { applyPaymentPreset(option.id); setImpactFocus("خطة التحصيل"); }} className="rounded-xl bg-white px-1 py-2 text-[10px] font-black text-indigo-700 ring-1 ring-indigo-100 transition hover:bg-indigo-100">{option.label}</button>)}</div><p className="mt-3 text-[11px] text-slate-500">مقدم {ppDownPct}% · تسليم {ppHandoverPct}%</p></div>}
                  </div>
                </section>
                ...
                <section data-testid="pricing-source-panel" className="overflow-hidden rounded-[22px] border-2 border-amber-300 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.07)]">
                  <div className="flex flex-col gap-3 border-b-2 border-amber-200 bg-[linear-gradient(115deg,#fffbeb,#ffffff_55%,#ecfeff)] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                    <div><p className="text-[11px] font-bold text-amber-700">لوحة التسعير المعتمدة</p><h2 className="mt-0.5 text-lg font-black text-slate-900">سعر القدم المربع هو نقطة الإدخال الوحيدة</h2><p className="mt-1 text-xs text-slate-600">عدد الوحدات ومساحتها مرجع من توزيع الوحدات؛ عدّل السعر فقط ليُحدّث الإيراد فورًا.</p></div>
                    <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-left"><p className="text-[10px] font-bold text-emerald-700">إجمالي الإيراد حسب التسعير</p><p className="mt-0.5 text-lg font-black tabular-nums text-emerald-800">{fmt(totalRevenue)} <span className="text-[10px]">AED</span></p></div>
                  </div>
                  <div className="grid gap-2.5 p-4 sm:grid-cols-2 xl:grid-cols-3">
                    {activeUnitRevenues.map((unit) => <article key={unit.id} data-testid="unit-pricing-card" className="rounded-2xl border-2 p-3.5 transition hover:-translate-y-0.5" style={{ borderColor: unit.color, background: `linear-gradient(135deg, ${unit.color}1c 0%, #ffffff 62%)`, boxShadow: `0 8px 22px ${unit.color}1a` }}><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-2"><span className="h-3 w-3 shrink-0 rounded-full ring-2 ring-white" style={{ backgroundColor: unit.color }} /><p className="truncate text-sm font-black text-slate-900">{unit.name}</p></div><span className="rounded-lg border bg-white/85 px-2 py-1 text-[10px] font-black" style={{ borderColor: `${unit.color}90`, color: unit.color }}>{fmtFull(unit.count)} وحدة</span></div><div className="mt-3 grid grid-cols-[1fr_1.25fr] gap-2"><div className="rounded-xl border-2 bg-white/85 px-2.5 py-2" style={{ borderColor: `${unit.color}70` }}><p className="text-[10px] font-bold text-slate-600">مساحة الوحدة</p><p className="mt-1 text-sm font-black tabular-nums text-slate-900">{fmtFull(unit.area)} <span className="text-[9px] text-slate-600">قدم²</span></p></div><label className="rounded-xl border-2 px-2.5 py-2 shadow-sm" style={{ borderColor: unit.color, backgroundColor: `${unit.color}14` }}><span className="block text-[10px] font-black" style={{ color: unit.color }}>سعر القدم² — قابل للتعديل</span><div className="mt-1 flex items-center gap-1"><input aria-label={`سعر ${unit.name}`} type="number" min={0} value={unit.price} onChange={(event) => { updateUnit(unit.id, "price", Number(event.target.value) || 0); setImpactFocus(`سعر ${unit.name}`); }} className="h-9 w-full min-w-0 rounded-lg border-2 bg-white px-2 text-center text-base font-black tabular-nums text-slate-900 outline-none transition focus:ring-4 focus:ring-amber-100" style={{ borderColor: unit.color }} /><span className="text-[9px] font-black" style={{ color: unit.color }}>AED</span></div></label></div><div className="mt-3 flex items-center justify-between border-t-2 pt-2.5" style={{ borderColor: `${unit.color}70` }}><span className="text-[10px] font-bold text-slate-600">إيراد هذا النوع</span><span className="text-sm font-black tabular-nums" style={{ color: unit.color }}>{fmt(unit.total)} AED</span></div></article>)}
                  </div>
                </section>

                <section data-testid="sales-calendar-board" className="overflow-hidden rounded-[22px] border border-slate-300 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.07)]">
                  <div className="flex flex-col gap-3 border-b border-slate-200 bg-slate-50/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                    <div><p className="text-[11px] font-bold text-blue-700">تقويم البيع المباشر</p><h2 className="mt-0.5 text-lg font-black text-slate-900">لوحة بيع 12 شهرًا</h2><p className="mt-1 text-xs text-slate-500">اكتب عدد الوحدات أو النسبة داخل البطاقة نفسها؛ الآخر يتزامن لحظيًا.</p></div>
                    {(() => { const maxPage = Math.max(0, Math.ceil(salesMonths / 12) - 1); const page = Math.min(salesCalendarPage, maxPage); const pageStart = page * 12; return <div className="flex items-center gap-2"><Button type="button" variant="outline" size="icon" aria-label="عرض أشهر البيع السابقة" disabled={page === 0} onClick={() => setSalesCalendarPage(Math.max(0, page - 1))} className="h-10 w-10 border-slate-300 bg-white text-slate-700 hover:bg-slate-100"><ChevronRight className="h-4 w-4" /></Button><span className="min-w-28 text-center text-sm font-black tabular-nums text-slate-800">الأشهر {pageStart + 1}–{Math.min(pageStart + 12, salesMonths)}</span><Button type="button" variant="outline" size="icon" aria-label="عرض أشهر البيع التالية" disabled={page === maxPage} onClick={() => setSalesCalendarPage(Math.min(maxPage, page + 1))} className="h-10 w-10 border-slate-300 bg-white text-slate-700 hover:bg-slate-100"><ChevronLeft className="h-4 w-4" /></Button></div>; })()}
                  </div>
                  <div className="p-4 sm:p-5">
                    {(() => {
                      ...
                      return <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">{visibleMonths.map((units, index) => { const absoluteMonth = salesStartMonth + pageStartIndex + index; const salesIndex = pageStartIndex + index; const startMonthIndex = projectStartDate ? Number(projectStartDate.split("-")[1]) - 1 : 0; const monthIndex = projectStartDate ? ((startMonthIndex + absoluteMonth - 1) % 12 + 12) % 12 : -1; const calendarYear = projectStartDate ? Number(projectStartDate.split("-")[0]) + Math.floor((startMonthIndex + absoluteMonth - 1) / 12) : null; const monthLabel = monthIndex >= 0 ? monthNames[monthIndex] : `شهر ${absoluteMonth}`; const selectedUnits = Math.max(0, Number(manualUnits[salesIndex] ?? units ?? 0) || 0); const percentage = offPlanUnits ? Math.round((selectedUnits / offPlanUnits) * 100) : 0; return <article key={absoluteMonth} className={`rounded-2xl border p-3.5 transition-all ${selectedUnits > 0 ? "border-emerald-300 bg-gradient-to-b from-white to-emerald-50/50 shadow-sm" : "border-slate-300 bg-slate-50/70"}`}><div className="flex items-start justify-between"><div><p className="text-sm font-black text-slate-800">{monthLabel}</p>{calendarYear && <p className="mt-0.5 text-[10px] font-black tabular-nums tracking-wide text-slate-500">{calendarYear}</p>}<p className="mt-0.5 text-[10px] font-semibold text-slate-500">شهر المشروع {absoluteMonth}</p></div><span className="rounded-lg bg-slate-900 px-2 py-1 text-[10px] font-black text-white">{percentage}%</span></div><div className="mt-4 grid grid-cols-2 gap-2"><label><span className="mb-1.5 block text-[10px] font-bold text-slate-600">الوحدات</span><input aria-label={`وحدات ${monthLabel}`} type="number" min={0} max={offPlanUnits} value={selectedUnits} onChange={(event) => { updateSalesMonth(salesIndex, Number(event.target.value) || 0); setImpactFocus(`${monthLabel} — الوحدات`); }} className="h-12 w-full rounded-xl border border-slate-400 bg-white text-center text-xl font-black text-emerald-700 outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100" /></label><label><span className="mb-1.5 block text-[10px] font-bold text-slate-600">النسبة</span><div className="relative"><input aria-label={`نسبة ${monthLabel}`} type="number" min={0} max={100} value={percentage} onChange={(event) => { updateSalesMonth(salesIndex, Math.round((Math.max(0, Number(event.target.value) || 0) / 100) * offPlanUnits)); setImpactFocus(`${monthLabel} — النسبة`); }} className="h-12 w-full rounded-xl border border-blue-300 bg-blue-50 text-center text-xl font-black text-blue-800 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100" /><span className="pointer-events-none absolute left-3 top-3.5 text-sm font-black text-blue-500">%</span></div></label></div><div className="mt-4 h-2.5 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-gradient-to-l from-emerald-500 to-teal-400 transition-all" style={{ width: `${Math.min(100, Math.max(0, (selectedUnits / maxUnitsInView) * 100))}%` }} /></div><p className="mt-2 text-center text-[10px] font-semibold text-slate-600">{fmt(selectedUnits * avgUnitPrice)} AED قيمة بيع متوقعة</p></article>; })}</div>;
                    })()}
                  </div>
                </section>

                <section data-testid="collection-cash-reading" className="overflow-hidden rounded-[18px] border border-slate-300 bg-white shadow-[0_8px_20px_rgba(15,23,42,0.06)]">
                  <div className="flex flex-wrap items-start justify-between gap-2.5 border-b border-slate-200 bg-slate-50/70 px-4 py-3"><div><p className="text-[10px] font-bold text-blue-700">قراءة مرئية دائمًا</p><h2 className="mt-0.5 text-base font-black text-slate-900">{isBuildForSale ? "الاستلام المباشر والأثر النقدي" : "التحصيل والإسكرو حسب شهر البيع"}</h2><p className="mt-1 text-[11px] leading-4 text-slate-600">{isBuildForSale ? "التحصيل الفعلي يساوي قيمة البيع لأن كامل حصيلة الوحدة تدخل للمستثمر عند البيع بعد الإنجاز." : "التحصيل الفعلي هو ما دخل إلى الإسكرو من الدفعات، وقد يختلف عن قيمة الوحدات المباعة في الشهر نفسه."}</p></div><Badge className={isBuildForSale ? "border border-indigo-200 bg-indigo-50 px-2 py-0.5 text-[10px] text-indigo-800 hover:bg-indigo-50" : hasDeficit ? "border border-red-200 bg-red-50 px-2 py-0.5 text-[10px] text-red-800 hover:bg-red-50" : "border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[10px] text-emerald-800 hover:bg-emerald-50"}>{isBuildForSale ? "استلام مباشر" : hasDeficit ? "تنبيه سيولة إسكرو" : "سيولة الإسكرو مريحة"}</Badge></div><div className="grid gap-2 p-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">{cashInflowData.filter((row) => row.cashInflow > 0 || row.salesThisMonth > 0).slice(0, 8).map((row) => <article key={row.month} data-testid="collection-cash-card" className="rounded-xl border border-slate-300 bg-white p-2.5"><div className="flex items-center justify-between gap-2"><p className="text-[11px] font-black text-slate-800">{formatCalendarMonth(row.month)}</p><span className="rounded-full bg-blue-50 px-1.5 py-0.5 text-[9px] font-bold text-blue-800">{isBuildForSale ? "مباشر" : "إسكرو"}</span></div><p className="mt-2 text-[9px] font-bold text-slate-600">{isBuildForSale ? "الاستلام الفعلي" : "التحصيل الفعلي"}</p><p className="mt-0.5 text-[15px] font-black tabular-nums text-blue-700">{fmt(row.cashInflow)} <span className="text-[8px]">AED</span></p><div className="mt-2 border-t border-slate-200 pt-1.5"><p className="text-[9px] font-bold text-slate-600">قيمة البيع في الشهر</p><p className="mt-0.5 text-[11px] font-black tabular-nums text-slate-800">{fmt(row.salesThisMonth)} <span className="text-[8px]">AED</span></p></div></article>)}</div>
                </section>

                <section data-testid="collection-cash-reading-compact" className="collection-cash-reading-compact overflow-hidden rounded-[18px] border-2 border-blue-200 bg-white shadow-[0_8px_20px_rgba(59,130,246,0.08)]">
                  <div className="flex flex-wrap items-center justify-between gap-2 border-b border-blue-200 bg-[linear-gradient(115deg,#eff6ff,#ffffff_55%,#ecfdf5)] px-3 py-2.5"><div><p className="text-[9px] font-black text-blue-700">قراءة مرئية دائمًا</p><h2 className="mt-0.5 text-sm font-black text-slate-900">{isBuildForSale ? "الاستلام المباشر حسب شهر البيع" : "التحصيل والإسكرو حسب شهر البيع"}</h2></div><Badge className={isBuildForSale ? "border border-indigo-200 bg-indigo-50 px-2 py-0.5 text-[9px] text-indigo-800 hover:bg-indigo-50" : hasDeficit ? "border border-red-200 bg-red-50 px-2 py-0.5 text-[9px] text-red-800 hover:bg-red-50" : "border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[9px] text-emerald-800 hover:bg-emerald-50"}>{isBuildForSale ? "استلام مباشر" : hasDeficit ? "تنبيه إسكرو" : "سيولة الإسكرو مريحة"}</Badge></div><div className="grid grid-cols-2 gap-1.5 p-2 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7">{visibleCollectionRows.map((row, index) => <article key={row.month} data-testid="collection-cash-tile" className={`rounded-xl border p-2 shadow-[0_2px_8px_rgba(15,23,42,0.04)] ${collectionPastelTones[index % collectionPastelTones.length]}`}><div className="flex items-center justify-between gap-1"><p className="text-[9px] font-black whitespace-nowrap">{formatCalendarMonth(row.month)}</p><span className="rounded-full bg-white/75 px-1 py-0.5 text-[8px] font-bold">{isBuildForSale ? "مباشر" : "إسكرو"}</span></div><div className="mt-1.5 border-t border-current/15 pt-1.5"><p className="text-[8px] font-bold opacity-70">{isBuildForSale ? "استلام" : "تحصيل"}</p><p className="mt-0.5 text-[13px] font-black tabular-nums leading-none">{fmt(row.cashInflow)} <span className="text-[7px]">AED</span></p></div><div className="mt-1.5 flex items-baseline justify-between gap-1"><span className="text-[8px] font-bold opacity-70">بيع</span><span className="text-[10px] font-black tabular-nums">{fmt(row.salesThisMonth)}</span></div></article>)}</div>
                </section>
              ...
              <aside className="xl:sticky xl:top-4">
                <section aria-hidden="true" className="hidden overflow-hidden rounded-[22px] border border-slate-800 bg-[#101b2d] text-white shadow-[0_16px_40px_rgba(15,23,42,0.22)]">
                  <div className="bg-[radial-gradient(circle_at_100%_0%,rgba(167,139,250,0.32),transparent_45%)] px-5 py-5"><div className="flex items-start justify-between gap-3"><div><p className="text-[11px] font-bold text-violet-200">أثر القرار — مباشر</p><h2 className="mt-1 text-xl font-black">ما الذي تغيّر الآن؟</h2><p className="mt-1 text-xs leading-5 text-slate-300">يراقب {impactFocus} ويُحدَّث دون انتظار الحفظ.</p></div><div className="rounded-xl bg-white/10 p-2.5"><BarChart3 className="h-5 w-5 text-violet-200" /></div></div></div>
                  <div className="space-y-3 px-4 pb-4"><div className="grid grid-cols-2 gap-3"><div className="rounded-2xl border border-emerald-300/20 bg-emerald-400/10 p-3.5"><p className="text-[11px] font-bold text-emerald-100">إيراد السيناريو</p><p className="mt-2 text-2xl font-black tracking-tight text-emerald-300">{fmt(totalRevenue)}</p><p className="mt-1 text-[10px] font-semibold text-emerald-100/70">AED</p></div><div className="rounded-2xl border border-violet-300/20 bg-violet-400/10 p-3.5"><p className="text-[11px] font-bold text-violet-100">الربح المتوقع</p><p className={`mt-2 text-2xl font-black tracking-tight ${profit >= 0 ? "text-violet-200" : "text-red-300"}`}>{fmt(profit)}</p><p className="mt-1 text-[10px] font-semibold text-violet-100/70">AED</p></div></div><div className="rounded-2xl border border-white/10 bg-white/[0.06] p-4"><p className="text-[11px] font-bold text-slate-300">أول تحصيل فعلي</p><div className="mt-2 flex items-end justify-between"><p className="text-2xl font-black text-white">{firstCollection ? `شهر ${firstCollection.month}` : "—"}</p><p className="text-sm font-bold text-blue-200">{firstCollection ? `${fmt(firstCollection.cashInflow)} AED` : "لا تحصيل بعد"}</p></div></div>{!isBuildForSale && <div className={`rounded-2xl border p-4 ${hasDeficit ? "border-red-400/30 bg-red-500/10" : "border-emerald-400/30 bg-emerald-500/10"}`}><p className="text-[11px] font-bold text-slate-200">أدنى رصيد في الإسكرو</p><div className="mt-2 flex items-end justify-between"><p className={`text-2xl font-black ${hasDeficit ? "text-red-300" : "text-emerald-300"}`}>{criticalMonth ? fmt(criticalMonth.balance) : "—"}</p><p className="text-sm font-bold text-slate-300">{criticalMonth ? `شهر ${criticalMonth.month}` : "لا توجد بيانات"}</p></div><p className={`mt-2 text-[11px] font-semibold ${hasDeficit ? "text-red-200" : "text-emerald-200"}`}>{hasDeficit ? "تنبيه: سيناريو البيع والتحصيل لا يغطي سحب الإسكرو في هذه النقطة." : "الرصيد لا يهبط إلى عجز وفق المدخلات الحالية."}</p></div>}<div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4"><div className="flex items-center justify-between"><p className="text-[11px] font-bold text-slate-300">نبض الأشهر المؤثرة</p><span className="text-[10px] font-bold text-slate-400">بيع / تحصيل</span></div><div className="mt-3 flex h-20 items-end gap-1.5">{visibleImpactMonths.length > 0 ? visibleImpactMonths.map((row) => { const maxCash = Math.max(...visibleImpactMonths.map((item) => item.cashInflow), 1); return <div key={row.month} className="group relative flex flex-1 flex-col items-center justify-end"><span className="mb-1 text-[9px] font-bold text-blue-200 opacity-0 transition group-hover:opacity-100">{fmt(row.cashInflow)}</span><div className="w-full rounded-t-md bg-gradient-to-t from-blue-600 to-cyan-300" style={{ height: `${Math.max(10, (row.cashInflow / maxCash) * 100)}%` }} /><span className="mt-1 text-[9px] font-bold text-slate-400">{row.month}</span></div>; }) : <p className="w-full text-center text-xs text-slate-400">أدخل توزيع البيع لتظهر الأشهر المؤثرة.</p>}</div></div><p className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-center text-[11px] font-semibold text-slate-300">التفاصيل الموسعة تحت التقويم؛ هذا الملخص ثابت أثناء أي تعديل.</p></div>
                </section>
            ...
            {false && <section className="rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm">
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
                ...
                <Button variant="outline" size="sm" className="gap-1.5"><LayoutDashboard className="h-3.5 w-3.5" />العودة للوحة السيناريو</Button>
              </div>
              ...
              <div className="p-3">
                {(() => {
                  ...
                  return <div className="mb-3 rounded-2xl border border-slate-200 bg-slate-50 p-3">
                    <div className="mb-3 flex items-center justify-between gap-2"><div><p className="text-[11px] font-black text-slate-800">لوحة البيع التفاعلية</p><p className="text-[9px] text-slate-500">12 شهرًا واضحًا؛ اكتب الوحدات أو النسبة داخل الشهر نفسه</p></div><div className="flex items-center gap-1"><Button type="button" size="sm" variant="outline" disabled={page === 0} onClick={() => setSalesCalendarPage(Math.max(0, page - 1))} className="h-7 px-2 text-[10px]">التالي</Button><span className="min-w-16 text-center text-[9px] font-bold text-slate-500">{page + 1} / {maxPage + 1}</span><Button type="button" size="sm" variant="outline" disabled={page === maxPage} onClick={() => setSalesCalendarPage(Math.min(maxPage, page + 1))} className="h-7 px-2 text-[10px]">السابق</Button></div></div>
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6 xl:grid-cols-12">{visibleMonths.map((units, index) => { const absoluteMonth = salesStartMonth + pageStartIndex + index; const salesIndex = pageStartIndex + index; const monthIndex = projectStartDate ? ((Number(projectStartDate.split("-")[1]) - 1 + absoluteMonth - 1) % 12 + 12) % 12 : -1; const selectedUnits = manualUnits[salesIndex] ?? units ?? 0; const percentage = offPlanUnits ? Math.round((selectedUnits / offPlanUnits) * 100) : 0; return <div key={absoluteMonth} className={`rounded-xl border p-2 text-center transition ${selectedUnits > 0 ? "border-emerald-200 bg-white shadow-sm" : "border-slate-200 bg-white/60"}`}><p className="text-[9px] font-black text-slate-600">{monthIndex >= 0 ? monthNames[monthIndex] : `شهر ${absoluteMonth}`}</p><div className="mt-2 space-y-1"><label className="block text-[7px] font-bold text-slate-400">وحدات</label><input aria-label={`وحدات ${monthIndex >= 0 ? monthNames[monthIndex] : `شهر ${absoluteMonth}`}`} type="number" min={0} max={offPlanUnits} value={selectedUnits} onChange={(event) => updateSalesMonth(salesIndex, Number(event.target.value) || 0)} className="h-8 w-full rounded-md border border-slate-200 bg-slate-50 text-center text-sm font-black text-emerald-700 outline-none transition focus:border-emerald-400 focus:bg-white focus:ring-2 focus:ring-emerald-100" /></div><div className="mt-2 space-y-1"><label className="block text-[7px] font-bold text-slate-400">النسبة %</label><input aria-label={`نسبة ${monthIndex >= 0 ? monthNames[monthIndex] : `شهر ${absoluteMonth}`}`} type="number" min={0} max={100} value={percentage} onChange={(event) => updateSalesMonth(salesIndex, Math.round((Math.max(0, Number(event.target.value) || 0) / 100) * offPlanUnits))} className="h-7 w-full rounded-md border border-emerald-200 bg-emerald-50 text-center text-xs font-black text-emerald-800 outline-none transition focus:border-emerald-400 focus:bg-white focus:ring-2 focus:ring-emerald-100" /></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-emerald-500" style={{ width: `${Math.min(100, Math.max(0, (selectedUnits / Math.max(...salesDistribution, 1)) * 100))}%` }} /></div></div>; })}</div>
                  </div>;
                })()}
                ...
                {!showSalesPrecision && <div className="mt-3 rounded-lg border border-dashed border-slate-200 bg-slate-50 px-3 py-2 text-[10px] text-slate-600"><strong className="text-slate-800">أدخل مباشرة داخل بطاقات الأشهر الظاهرة.</strong> كل تعديل للوحدات أو النسبة يحدّث الآخر فورًا. افتح المحرر الكامل فقط عندما تريد مراجعة كل أشهر المشروع دفعة واحدة.</div>}
